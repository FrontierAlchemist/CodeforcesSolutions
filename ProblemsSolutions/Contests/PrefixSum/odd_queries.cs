...
/// <summary>
/// Solve https://codeforces.com/problemset/problem/1807/D problem.
/// </summary>
internal class Solver
{
	...
	private static void Solve()
	{
		...
		for (int i = 1; i <= arraySize; ++i) {
			prefixSums[i] = prefixSums[i - 1] + Input.ReadLong();
		}
		for (int q = 0; q < queriesCount; ++q) {
			int leftPoint = Input.ReadInt();
			int rightPoint = Input.ReadInt();
			long newValue = Input.ReadLong();
			int segmentSize = rightPoint - leftPoint + 1;
			long oldSegmentSum = prefixSums[rightPoint] - prefixSums[leftPoint - 1];
			long newSegmentSum = segmentSize * newValue;
			long sumDifference = newSegmentSum - oldSegmentSum;
			long newTotalSum = prefixSums[arraySize] + sumDifference;
			Output.WriteLine(newTotalSum % 2 == 1 ? "YES" : "NO");
		}
	}
}
