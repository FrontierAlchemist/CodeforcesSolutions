...
/// <summary>
/// Solve https://codeforces.com/problemset/problem/1899/B problem.
/// </summary>
internal class Solver
{
	...
	{
		int boxesCount = Input.ReadInt();
		int[] boxesWeights = Input.ReadIntArray(boxesCount);
		long[] sumPrefixes = new long[boxesCount + 1];
		for (int i = 1; i <= boxesCount; ++i) {
			sumPrefixes[i] = sumPrefixes[i - 1] + boxesWeights[i - 1];
		}
		...
		for (int k = 1; k < boxesCount; ++k) {
			...
			long minTruckWeight = long.MaxValue;
			long maxTruckWeight = 0L;
			for (int i = 0; i < boxesCount; i += k) {
				long truckWeight = sumPrefixes[i + k] - sumPrefixes[i];
				minTruckWeight = Math.Min(truckWeight, minTruckWeight);
				maxTruckWeight = Math.Max(truckWeight, maxTruckWeight);
			}
			answer = Math.Max(maxTruckWeight - minTruckWeight, answer);
		}
