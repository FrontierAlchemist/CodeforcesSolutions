...
/// <summary>
/// Solve https://codeforces.com/problemset/problem/1927/D problem.
/// </summary>
internal class Solver
{
	...
	private static void Solve()
	{
		...
		int[] positionOfNextDiffierentNumber = new int[arraySize];
		int startPositionOfEqualNumbersSegments = 0;
		int previousNumber = Input.ReadInt();
		for (int i = 1; i < arraySize; ++i) {
			int currentNumber = Input.ReadInt();
			if (previousNumber != currentNumber) {
				for (int j = startPositionOfEqualNumbersSegments; j < i; ++j) {
					positionOfNextDiffierentNumber[j] = i;
				}
				startPositionOfEqualNumbersSegments = i;
			}
		...
		for (int i = 0; i < queriesCount; ++i) {
			int left = Input.ReadInt() - 1;
			int right = Input.ReadInt() - 1;

			if (positionOfNextDiffierentNumber[left] > right || positionOfNextDiffierentNumber[left] == 0) {
				Output.WriteLine("-1 -1");
			} else {
				Output.WriteLine($"{left + 1} {positionOfNextDiffierentNumber[left] + 1}");
			}
		}
		Output.WriteLine();
	}
