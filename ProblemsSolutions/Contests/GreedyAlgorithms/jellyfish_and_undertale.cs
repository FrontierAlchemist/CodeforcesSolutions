...
/// <summary>
/// Solve https://codeforces.com/problemset/problem/1875/A problem.
/// </summary>
internal class Solver
{
	...
	{
		int maximalTimerValue = Input.ReadInt();
		int currentTimerValue = Input.ReadInt();
		int toolsCount = Input.ReadInt();
		int[] toolsValues = Input.ReadIntArray(toolsCount);

		long totalTimeUntilExplosion = currentTimerValue;
		for (int i = 0; i < toolsCount; ++i) {
			int newTimerValue = toolsValues[i] + 1;
			if (newTimerValue <= maximalTimerValue) {
				totalTimeUntilExplosion += toolsValues[i];
			} else {
				totalTimeUntilExplosion += maximalTimerValue - 1;
			}
		}

		Output.WriteLine(totalTimeUntilExplosion);
	}
}
