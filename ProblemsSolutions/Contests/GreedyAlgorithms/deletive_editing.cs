...
/// <summary>
/// Solve https://codeforces.com/problemset/problem/1666/D problem.
/// </summary>
internal class Solver
{
	...
	private static void Solve()
	{
		...
		if (targetString.Length <= sourceString.Length) {
			HashSet<char> removedLetters = new();
			int targetStringIndex = targetString.Length - 1;
			for (int i = sourceString.Length - 1; i >= 0; --i) {
				if (sourceString[i] == targetString[targetStringIndex]) {
					--targetStringIndex;
					if (targetStringIndex == -1) {
						isTargetReachable = true;
						break;
					}
					if (removedLetters.Contains(targetString[targetStringIndex])) {
						break;
					}
				} else {
					removedLetters.Add(sourceString[i]);
				}
			}
		}

		Output.WriteLine(isTargetReachable ? "YES" : "NO");
	}
}
