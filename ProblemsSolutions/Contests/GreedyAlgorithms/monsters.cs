...
/// <summary>
/// Solve https://codeforces.com/problemset/problem/1849/B problem.
/// </summary>
internal class Solver
{
	...
	{
		int monstersCount = Input.ReadInt();
		int damage = Input.ReadInt();
		(int health, int index)[] monsters = new (int, int)[monstersCount];
		for (int i = 0; i < monstersCount; ++i) {
			int monsterHealth = Input.ReadInt();
			int finalHealth = monsterHealth % damage;
			if (finalHealth == 0) {
				finalHealth = damage;
			}
			monsters[i] = (finalHealth, i + 1);
		}
		Array.Sort(monsters, (a, b) => {
			int healthComparision = b.health.CompareTo(a.health);
			return healthComparision == 0 ? a.index.CompareTo(b.index) : healthComparision;
		});
		for (int i = 0; i < monstersCount - 1; ++i) {
			Output.Write($"{monsters[i].index} ");
		}
		Output.WriteLine($"{monsters[monstersCount - 1].index}");
	}
}
