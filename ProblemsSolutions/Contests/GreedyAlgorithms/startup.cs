...
/// <summary>
/// Solve https://codeforces.com/contest/2036/problem/B problem.
/// </summary>
public class ProblemSolver
{
	...
	private void SolveTestCase()
	{
		...
		int shelvesCount = int.Parse(valuesFromInput[0]);
		int bottlesCount = int.Parse(valuesFromInput[1]);
		long[] brandsValues = new long[bottlesCount];
		for (int i = 0; i < bottlesCount; ++i) {
			valuesFromInput = input.ReadLineAndSplit();
			int brand = int.Parse(valuesFromInput[0]);
			int cost = int.Parse(valuesFromInput[1]);
			brandsValues[brand - 1] += cost;
		}

		Array.Sort(brandsValues);

		long maximalTotalValue = 0L;
		for (int i = bottlesCount - 1; i >= 0; --i) {
			maximalTotalValue += brandsValues[i];
			if (--shelvesCount == 0) {
				break;
			}
		}

		output.WriteLine(maximalTotalValue);
	}
}
