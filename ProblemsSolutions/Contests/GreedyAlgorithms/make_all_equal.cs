...
/// <summary>
/// Solve https://codeforces.com/problemset/problem/2001/A problem.
/// </summary>
internal class Solver
{
	...
	private static void Solve()
	{
		...
		int[] occurrences = new int[arraySize + 1];
		for (int i = 0; i < arraySize; ++i) {
			int arrayElement = Input.ReadInt();
			++occurrences[arrayElement];
		}

		int largestOccurrence = 0;
		for (int i = 1; i <= arraySize; ++i) {
			largestOccurrence = Math.Max(largestOccurrence, occurrences[i]);
		}

		int minimalRemovingsCount = arraySize - largestOccurrence;
		Output.WriteLine(minimalRemovingsCount);
	}
}
