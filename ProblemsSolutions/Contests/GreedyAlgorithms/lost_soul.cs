...
/// <summary>
/// Solve https://codeforces.com/contest/2117/problem/E problem.
/// </summary>
internal class Solver
{
	...
	private static void Solve()
	{
		...
		int maximalEqualPairsCount = 0;
		if (a[^1] == b[^1]) {
			maximalEqualPairsCount = arraySize;
		} else {
			HashSet<int> pullOfNumbers = new();
			for (int i = arraySize - 2; i >= 0; --i) {
				if (
					a[i] == b[i] ||
					a[i] == a[i + 1] ||
					b[i] == b[i + 1] ||
					pullOfNumbers.Contains(a[i]) ||
					pullOfNumbers.Contains(b[i])
				) {
					maximalEqualPairsCount = i + 1;
					break;
				}
				pullOfNumbers.Add(a[i + 1]);
				pullOfNumbers.Add(b[i + 1]);
			}
		}

		Output.WriteLine(maximalEqualPairsCount);
	}
}
