...
/// <summary>
/// Solve https://codeforces.com/contest/1726/problem/A problem.
/// </summary>
internal class Solver
{
	...
	private static void Solve()
	{
		...
		if (arraySize == 1) {
			...
			answer = Math.Max(array[1] - array[0], array[0] - array[1]);
		}
		else {
			answer = array[arraySize - 1] - array[0];
			answer = Math.Max(answer, Math.Max(array[1] - array[0], array[arraySize - 1] - array[1]));
			answer = Math.Max(answer, Math.Max(array[0] - array[1], array[arraySize - 2] - array[arraySize - 1]));
			for (int i = 2; i < arraySize - 1; ++i) {
				answer = Math.Max(answer, array[i - 1] - array[i]);
				answer = Math.Max(answer, Math.Max(array[i] - array[0], array[arraySize - 1] - array[i]));
			}
		}
