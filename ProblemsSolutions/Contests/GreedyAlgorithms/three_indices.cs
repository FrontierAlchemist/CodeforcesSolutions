...
/// <summary>
/// Solve https://codeforces.com/problemset/problem/1380/A problem.
/// </summary>
internal class Solver
{
	...
	private static void Solve()
	{
		...
		int[] indexOfMinimumOnSuffix = new int[permutationLength];
		indexOfMinimumOnSuffix[permutationLength - 1] = permutationLength - 1;
		for (int p = permutationLength - 2; p >= 0; --p) {
			int k = indexOfMinimumOnSuffix[p + 1];
			indexOfMinimumOnSuffix[p] = permutation[p] < permutation[k] ? p : k;
		}

		int i = 0;
		for (int j = 1; j < permutationLength - 1; ++j) {
			int k = indexOfMinimumOnSuffix[j + 1];
			if (permutation[i] < permutation[j] && permutation[j] > permutation[k]) {
