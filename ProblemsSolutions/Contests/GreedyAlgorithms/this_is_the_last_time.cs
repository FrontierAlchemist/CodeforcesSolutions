...
/// <summary>
/// Solve https://codeforces.com/contest/2126/problem/D problem.
/// </summary>
internal class Solver
{
	...
	{
		int casinosCount = Input.ReadInt();
		int startBalance = Input.ReadInt();

		List<CasinoInfo> casinosInfos = new(casinosCount);
		for (int i = 0; i < casinosCount; ++i) {
			casinosInfos.Add(new() {
				Left = Input.ReadInt(),
				Right = Input.ReadInt(),
				EndBalance = Input.ReadInt()
			});
		}

		casinosInfos.Sort((a, b) => a.Left.CompareTo(b.Left));

		long currentBalance = startBalance;
		foreach (var info in casinosInfos) {
			if (info.Left > currentBalance) {
				break;
			}
			currentBalance = Math.Max(currentBalance, info.EndBalance);
		}

		Output.WriteLine(currentBalance);
	}
}
