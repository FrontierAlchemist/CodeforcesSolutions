...
/// <summary>
/// Solve https://codeforces.com/problemset/problem/1791/G1 problem.
/// </summary>
internal class Solver
{
	...
	{
		int teleportsCount = Input.ReadInt();
		int balance = Input.ReadInt();
		int[] costs = new int[teleportsCount];
		for (int i = 0; i < teleportsCount; ++i) {
			costs[i] = Input.ReadInt() + i + 1;
		}

		Array.Sort(costs);

		int usedTeleportsCount = 0;
		for (int i = 0; i < teleportsCount; ++i) {
			if (costs[i] <= balance) {
				balance -= costs[i];
				++usedTeleportsCount;
		...
		Output.WriteLine(usedTeleportsCount);
	}
}
