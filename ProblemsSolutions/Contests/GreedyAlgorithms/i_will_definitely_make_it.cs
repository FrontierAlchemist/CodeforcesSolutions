...
/// <summary>
/// Solve https://codeforces.com/contest/2126/problem/C problem.
/// </summary>
internal class Solver
{
	...
	{
		int towersCount = Input.ReadInt();
		int startTowerNumber = Input.ReadInt();
		int[] towersHeights = Input.ReadIntArray(towersCount);
		int startTowerHeight = towersHeights[startTowerNumber - 1];
		Array.Sort(towersHeights);
		bool isPossibleToSurvive = true;
		int waterLevel = 0;
		for (int i = 0; i < towersCount - 1; ++i) {
			if (towersHeights[i + 1] <= startTowerHeight) {
				continue;
			}
			int heightDifference = towersHeights[i + 1] - towersHeights[i];
			if (waterLevel + heightDifference > towersHeights[i]) {
				isPossibleToSurvive = false;
				break;
			}
			waterLevel += heightDifference;
		}
		Output.WriteLine(isPossibleToSurvive ? "YES" : "NO");
	}
}
