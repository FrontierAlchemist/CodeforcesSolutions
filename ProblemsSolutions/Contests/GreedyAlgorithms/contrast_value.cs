...
/// <summary>
/// Solve https://codeforces.com/problemset/problem/1832/C problem.
/// </summary>
		...
		if (arraySize >= 2) {
			...
			while (next < arraySize) {
				if (array[next] == array[current]) {
					...
					long d1 = Math.Abs(array[proteced] - array[current]);
					long d2 = Math.Abs(array[current] - array[next]);
					long d3 = Math.Abs(array[proteced] - array[next]);
					if (d1 + d2 == d3) {
						current = next;
					} else {
						++modifiedArraySize;
			...
			if (array[proteced] != array[current]) {
				++modifiedArraySize;
			}
		}

		Output.WriteLine(modifiedArraySize);
	}
}
