...
/// <summary>
/// Solve https://codeforces.com/contest/2091/problem/B problem.
/// </summary>
internal class Solver
{
	...
	private static void Solve()
	{
		...
		int requiredStrength = Input.ReadInt();
		int[] studentsStrength = Input.ReadIntArray(studentsCount);
		Array.Sort(studentsStrength);

		int strongStudents = 0;
		for (int i = studentsCount - 1; i >= 0; --i) {
			if (studentsStrength[i] >= requiredStrength) {
				++strongStudents;
			} else {
				break;
			}
		...
		int answer = strongStudents;
		int studentsWithoutTeam = 0;
		for (int i = studentsCount - strongStudents - 1; i >= 0; --i) {
			++studentsWithoutTeam;
			int requiredTeamSize = Math.DivRem(requiredStrength, studentsStrength[i], out int remainder);
			if (remainder > 0) {
				++requiredTeamSize;
			}
			if (studentsWithoutTeam >= requiredTeamSize) {
				++answer;
