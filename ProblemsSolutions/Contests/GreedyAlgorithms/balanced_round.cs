...
/// <summary>
/// Solve https://codeforces.com/problemset/problem/1850/D problem.
/// </summary>
internal class Solver
{
	...
	{
		int problemsCount = Input.ReadInt();
		int differenceLimit = Input.ReadInt();
		int[] problemsDifficulties = Input.ReadIntArray(problemsCount);
		Array.Sort(problemsDifficulties);

		int currentCorrectSequenceLength = 1;
		int maximalCorrectSequenceLength = currentCorrectSequenceLength;
		for (int i = 1; i < problemsCount; ++i) {
			if (problemsDifficulties[i] - problemsDifficulties[i - 1] <= differenceLimit) {
				++currentCorrectSequenceLength;
			} else {
				maximalCorrectSequenceLength = Math.Max(maximalCorrectSequenceLength, currentCorrectSequenceLength);
				currentCorrectSequenceLength = 1;
			}
		}
		maximalCorrectSequenceLength = Math.Max(maximalCorrectSequenceLength, currentCorrectSequenceLength);

		Output.WriteLine(problemsCount - maximalCorrectSequenceLength);
	}
}
