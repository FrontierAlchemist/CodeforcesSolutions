...
/// <summary>
/// Solve https://codeforces.com/problemset/problem/1474/B problem.
/// </summary>
internal class Solver
{
	private const bool IsSeveralTests = true;

	private const int MaximalPrimeNumber = 20021;

	private static StreamReaderWrapper Input => Program.Input;
	private static StreamWriterWrapper Output => Program.Output;


	private static bool[] isPrimeNumber = new bool[MaximalPrimeNumber];
	...
	{
		isPrimeNumber = new bool[MaximalPrimeNumber + 1];
		Array.Fill(isPrimeNumber, true);
		for (int i = 2; i <= MaximalPrimeNumber; ++i) {
			if (!isPrimeNumber[i]) {
				continue;
			}
			for (int j = i * 2; j <= MaximalPrimeNumber; j += i) {
	...
	{
		int difference = Input.ReadInt();

		int firstDivisor = 1;
		int secondDivisor = firstDivisor + difference;
		for (int i = secondDivisor; i <= MaximalPrimeNumber; ++i) {
			if (isPrimeNumber[i]) {
				secondDivisor = i;
				break;
			}
		...
		int thirdDivisor = secondDivisor + difference;
		for (int i = thirdDivisor; i <= MaximalPrimeNumber; ++i) {
			if (isPrimeNumber[i]) {
				thirdDivisor = i;
				break;
			}
		...
		int fourthDivisor = secondDivisor * thirdDivisor;
		Output.WriteLine(fourthDivisor);
	}
}
