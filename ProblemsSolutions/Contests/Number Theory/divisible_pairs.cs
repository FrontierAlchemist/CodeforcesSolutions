...
/// <summary>
/// Solve https://codeforces.com/problemset/problem/1931/D problem.
/// </summary>
internal class Solver
{
	...
	private static void Solve()
	{
		...
		long beautifulPairs = 0L;
		Dictionary<int, Dictionary<int, int>> remaindersMapping = new();
		for (int i = 0; i < arraySize; ++i) {
			int number = Input.ReadInt();
			int remainderByX = number % x;
			int remainderByY = number % y;
			int keyByX = remainderByX == 0 ? 0 : x - remainderByX;
			if (remaindersMapping.TryGetValue(keyByX, out var remaindersByY)) {
				if (remaindersByY.TryGetValue(remainderByY, out int pairsCount)) {
					beautifulPairs += pairsCount;
				}
			}
			if (!remaindersMapping.TryGetValue(remainderByX, out var otherRemainderByY)) {
				remaindersMapping.Add(remainderByX, new() { { remainderByY, 1 } });
			} else {
				if (!otherRemainderByY.TryGetValue(remainderByY, out int pairsCount)) {
					otherRemainderByY.Add(remainderByY, 0);
				}
				otherRemainderByY[remainderByY] = pairsCount + 1;
			}
		}

		Output.WriteLine(beautifulPairs);
	}
}
