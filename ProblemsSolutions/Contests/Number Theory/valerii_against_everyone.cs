...
/// <summary>
/// Solve https://codeforces.com/problemset/problem/1438/B problem.
/// </summary>
internal class Solver
{
	...
	private static void Solve()
	{
		...
		bool isEqualNumbersExist = false;
		for (int i = 0; i < arraySize; ++i) {
			int number = Input.ReadInt();
			if (isEqualNumbersExist) {
				continue;
			}
			if (distinctNumbers.Contains(number)) {
				isEqualNumbersExist = true;
			} else {
				distinctNumbers.Add(number);
			}
		}
		Output.WriteLine(isEqualNumbersExist ? "YES" : "NO");
	}
}
