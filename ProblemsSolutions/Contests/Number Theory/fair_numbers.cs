...
/// <summary>
/// Solve https://codeforces.com/problemset/problem/1411/B problem.
/// </summary>
internal class Solver
{
	...
	{
		long sourceNumber = Input.ReadLong();
		while (!IsFairNumber(sourceNumber)) {
			++sourceNumber;
		}
		Output.WriteLine(sourceNumber);

		bool IsFairNumber(long checkedNumber)
		{
			long copy = checkedNumber;
			while (copy > 0) {
				long digit = copy % 10L;
				if (digit != 0L && checkedNumber % digit > 0L) {
					return false;
				}
				copy /= 10L;
			}
