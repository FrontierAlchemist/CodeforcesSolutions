...
/// <summary>
/// Solve https://codeforces.com/problemset/problem/1859/A problem.
/// </summary>
internal class Solver
{
	...
	private static void Solve()
	{
		...
		if (maxIndex == 0) {
			...
			Output.WriteLine($"{maxIndex} {arraySize - maxIndex}");
			for (int i = 0; i < maxIndex - 1; ++i) {
				Output.Write($"{array[i]} ");
			}
			Output.WriteLine(array[maxIndex - 1]);
			for (int i = maxIndex; i < arraySize - 1; ++i) {
				Output.Write($"{array[i]} ");
			}
			Output.WriteLine(array[maxIndex]);
		}
	}
}
