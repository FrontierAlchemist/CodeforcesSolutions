...
/// <summary>
/// Solve https://codeforces.com/contest/1828/problem/B problem.
/// </summary>
internal class Solver
{
	...
	private static void Solve()
	{
		...
		for (int i = 1; i <= permutationLength; ++i) {
			...
			int distanceToSourcePosition = Math.Abs(number - i);
			if (distanceToSourcePosition > 0) {
				gcd = gcd == -1 ? distanceToSourcePosition : Gcd(gcd, distanceToSourcePosition);
			}
		}
		Output.WriteLine(gcd);
	}

	private static int Gcd(int a, int b) => b == 0 ? a : (a >= b ? Gcd(b, a % b) : Gcd(a, b % a));
}
