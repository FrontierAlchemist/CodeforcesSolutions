...
/// <summary>
/// Solve https://codeforces.com/contest/2117/problem/D problem.
/// </summary>
internal class Solver
{
	...
	private static void Solve()
	{
		...
		long y = (2L * array[0] - array[1]) / (arraySize + 1);
		long x = y - array[0] + array[1];

		bool isPossibleToExplodeArray = x >= 0 && y >= 0;
		if (isPossibleToExplodeArray) {
			for (int i = 0; i < arraySize; ++i) {
				long modifiedNumber = array[i] - (i + 1) * x - (arraySize - i) * y;
				if (modifiedNumber != 0) {
					isPossibleToExplodeArray = false;
					break;
				}
			}
		}

		Output.WriteLine(isPossibleToExplodeArray ? "YES" : "NO");
	}
}
