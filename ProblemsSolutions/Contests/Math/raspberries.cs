...
/// <summary>
/// Solve https://codeforces.com/problemset/problem/1883/C problem.
/// </summary>
internal class Solver
{
	...
	private static void Solve() {
		...
		int divider = Input.ReadInt();
		int[] array = Input.ReadIntArray(arraySize);

		int minimalOperationsCount = divider;
		int numberDivisibleByTwo = 0;
		for (int i = 0; i < arraySize; ++i) {
			int remainder = array[i] % divider;
			if (remainder == 0) {
				minimalOperationsCount = 0;
				break;
			} else {
				if (divider == 4) {
					if (array[i] % 2 == 0) {
						++numberDivisibleByTwo;
					}
				}
				minimalOperationsCount = Math.Min(minimalOperationsCount, divider - remainder);
			}
		}

		if (divider == 4 && minimalOperationsCount > 0) {
			if (numberDivisibleByTwo >= 2) {
				minimalOperationsCount = 0;
			} else if (numberDivisibleByTwo == 1 && arraySize > 1) {
				minimalOperationsCount = 1;
			} else if (arraySize > 1 && minimalOperationsCount > 2) {
