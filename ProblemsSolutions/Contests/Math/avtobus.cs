...
/// <summary>
/// Solve https://codeforces.com/problemset/problem/1679/A problem.
/// </summary>
internal class Solver
{
	...
	{
		long totalWheelsCount = Input.ReadLong();

		bool isAnswerExist = totalWheelsCount >= 4 && totalWheelsCount % 2L == 0L;
		string verdict;
		if (isAnswerExist) {
			long minimalVehiclesCount = Math.DivRem(totalWheelsCount, 6L, out long remainWheels);
			if (remainWheels > 0) {
				++minimalVehiclesCount;
			}
			long maximalVehiclesCount = Math.DivRem(totalWheelsCount, 4L, out _);

			verdict = $"{minimalVehiclesCount} {maximalVehiclesCount}";
		} else {
			verdict = "-1";
		}
