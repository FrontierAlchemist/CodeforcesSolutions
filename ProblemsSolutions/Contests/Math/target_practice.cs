...
/// <summary>
/// Solve https://codeforces.com/problemset/problem/1873/C problem.
/// </summary>
internal class Solver
{
	...
	private static void Solve()
	{
		...
		const int HalfMatrixSize = 5;

		int totalScore = 0;
		for (int i = 0; i < MatrixSize; ++i) {
			string inputString = Input.ReadString();
			int row = i + 1 <= HalfMatrixSize ? i + 1 : MatrixSize - i;
			for (int j = 0; j < MatrixSize; ++j) {
				if (inputString[j] == 'X') {
					int column = j + 1 <= HalfMatrixSize ? j + 1 : MatrixSize - j;
					totalScore += Math.Min(row, column);
				}
			}
		}
