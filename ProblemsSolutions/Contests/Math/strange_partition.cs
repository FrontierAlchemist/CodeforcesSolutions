...
/// <summary>
/// Solve https://codeforces.com/problemset/problem/1471/A problem.
/// </summary>
internal class Solver
{
	...
	private static void Solve()
	{
		...
		int delimiter = Input.ReadInt();

		long arraySum = 0L;
		long maximalBeauty = 0L;
		for (int i = 0; i < arraySize; ++i) {
			int number = Input.ReadInt();
			arraySum += number;
			maximalBeauty += RoundUp(number, delimiter);
		}
		long minimalBeauty = RoundUp(arraySum, delimiter);

		Output.WriteLine($"{minimalBeauty} {maximalBeauty}");
	}

	private static long RoundUp(long numerator, long denominator)
	{
		long result = Math.DivRem(numerator, denominator, out long remainder);
		if (remainder > 0) {
			++result;
		}
		return result;
	...
	{
		int result = Math.DivRem(numerator, denominator, out int remainder);
		if (remainder > 0) {
			++result;
		}
		return result;
