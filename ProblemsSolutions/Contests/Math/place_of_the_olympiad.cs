...
/// <summary>
/// Solve https://codeforces.com/contest/2091/problem/D problem.
/// </summary>
internal class Solver
{
	...
	private static void Solve()
	{
		...
		int participantsCount = Input.ReadInt();

		int minimalLengthOfLongestBench = 1;
		int deskPerRow = Math.DivRem(participantsCount, rowsCount, out int remainDesks);
		if (remainDesks > 0) {
			++deskPerRow;
		}
		if (deskPerRow == columnsCount) {
			minimalLengthOfLongestBench = deskPerRow;
		} else {
			int freeSpots = columnsCount - deskPerRow;
			int benchesCount = freeSpots + 1;
			minimalLengthOfLongestBench = Math.DivRem(deskPerRow, benchesCount, out int remainBenches);
			if (remainBenches > 0) {
				++minimalLengthOfLongestBench;
			}
		}
		Output.WriteLine(minimalLengthOfLongestBench);
	}
}
