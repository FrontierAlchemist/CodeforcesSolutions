...
/// <summary>
/// Solve https://codeforces.com/problemset/problem/1881/A problem.
/// </summary>
internal class Solver
{
	...
	{
		int sourceStringLength = Input.ReadInt();
		int targetStringLength = Input.ReadInt();
		string sourceStrnig = Input.ReadString();
		string targetString = Input.ReadString();

		int operationsCount = 0;
		while (sourceStringLength < targetStringLength) {
			sourceStrnig += sourceStrnig;
			sourceStringLength *= 2;
			++operationsCount;
		}

		if (!sourceStrnig.Contains(targetString)) {
			sourceStrnig += sourceStrnig;
			sourceStringLength *= 2;
			++operationsCount;
			if (!sourceStrnig.Contains(targetString)) {
