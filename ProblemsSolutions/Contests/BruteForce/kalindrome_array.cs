...
/// <summary>
/// Solve https://codeforces.com/problemset/problem/1610/B problem.
/// </summary>
internal class Solver
{
	...
	private static void Solve()
	{
		...
		bool isKalindrome = true;
		for (int i = 0; i < arraySize; ++i) {
			if (array[i] != array[^(1 + i)]) {
				isKalindrome = IsPalindrome(array[i]) || IsPalindrome(array[^(1 + i)]);
				break;
			}
		}
		Output.WriteLine(isKalindrome ? "YES" : "NO");

		bool IsPalindrome(int excludedNumber)
		{
			List<int> modifiedArray = new();
			for (int i = 0; i < arraySize; ++i) {
				if (array[i] != excludedNumber) {
					modifiedArray.Add(array[i]);
				}
			}
			for (int i = 0; i < modifiedArray.Count / 2; ++i) {
				if (modifiedArray[i] != modifiedArray[^(1 + i)]) {
					return false;
				}
			}
			return true;
