...
/// <summary>
/// Solve https://codeforces.com/problemset/problem/1237/B problem.
/// </summary>
internal class Solver
{
	...
	{
		int carsCount = Input.ReadInt();
		int[] inOrder = Input.ReadIntArray(carsCount);
		int[] outOrder = Input.ReadIntArray(carsCount);

		bool[] outed = new bool[carsCount + 1];
		bool[] ined = new bool[carsCount + 1];
		HashSet<int> violators = new();
		...
		for (int i = 0; i < carsCount; ++i) {
			...
			while (outIndex < carsCount) {
				int carOut = outOrder[outIndex++];
				outed[carOut] = true;
				if (carOut == carIn) {
					break;
				}
				if (!ined[carOut]) {
					violators.Add(carOut);
				}
			}
			ined[carIn] = true;
		}

		Output.WriteLine(violators.Count);
	}
}
