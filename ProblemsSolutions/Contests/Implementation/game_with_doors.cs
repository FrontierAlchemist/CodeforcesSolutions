...
/// <summary>
/// Solve https://codeforces.com/problemset/problem/2004/B problem.
/// </summary>
internal class Solver
{
	...
	{
		const int RoomsCount = 100;

		(int left, int right) aliceSegment = (Input.ReadInt(), Input.ReadInt());
		(int left, int right) bobSegment = (Input.ReadInt(), Input.ReadInt());

		bool[] isOccupiedByAlice = new bool[RoomsCount + 2];
		for (int i = aliceSegment.left; i <= aliceSegment.right; ++i) {
			isOccupiedByAlice[i] = true;
		}

		int commonRooms = 0;
		for (int i = bobSegment.left; i <= bobSegment.right; ++i) {
			if (isOccupiedByAlice[i]) {
				++commonRooms;
			}
		}
		...
		int doorsToClose = 1;
		if (commonRooms > 0) {
			doorsToClose = commonRooms - 1;
			if (bobSegment.left != aliceSegment.left) {
				++doorsToClose;
			}
			if (bobSegment.right != aliceSegment.right) {
				++doorsToClose;
			}
		}

		Output.WriteLine(doorsToClose);
	}
}
