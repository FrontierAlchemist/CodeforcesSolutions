...
internal class Solver
{
	...
	/// <summary>
	/// Solve https://codeforces.com/problemset/problem/237/A problem.
	/// </summary>
	public static void SolveProblem()
	{
		int customersCount = Input.ReadInt();
		int currentCustomersCount = 0;
		int lastCustomerTime = -1;
		int neededCashesCount = 1;
		for (int i = 0; i < customersCount; ++i) {
			int hour = Input.ReadInt();
			int minute = Input.ReadInt();
			int currentTime = hour * 60 + minute;
			if (currentTime == lastCustomerTime) {
				++currentCustomersCount;
			} else {
				neededCashesCount = Math.Max(neededCashesCount, currentCustomersCount);
				currentCustomersCount = 1;
				lastCustomerTime = currentTime;
			}
		}
		neededCashesCount = Math.Max(neededCashesCount, currentCustomersCount);
		Output.WriteLine(neededCashesCount);
	}
}
