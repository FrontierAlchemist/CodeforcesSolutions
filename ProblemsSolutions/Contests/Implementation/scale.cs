...
/// <summary>
/// Solve https://codeforces.com/problemset/problem/1996/B problem.
/// </summary>
internal class Solver
{
	...
	private static void Solve()
	{
		...
		int newMatrixSize = matrixSize / divisor;
		char[,] newMatrix = new char[newMatrixSize, newMatrixSize];
		for (int i = 0; i < matrixSize; ++i) {
			string line = Input.ReadString();
			for (int j = 0; j < matrixSize; ++j) {
				if ((i + 1) % divisor == 0 && (j + 1) % divisor == 0) {
					newMatrix[i / divisor, j / divisor] = line[j];
				}
			}
		}
		for (int i = 0; i < newMatrixSize; ++i) {
			for (int j = 0; j < newMatrixSize; ++j) {
				Output.Write(newMatrix[i, j]);
			}
			Output.WriteLine();
		}
	}
