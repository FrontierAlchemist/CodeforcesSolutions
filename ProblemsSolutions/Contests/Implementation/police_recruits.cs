...
/// <summary>
/// Solve https://codeforces.com/problemset/problem/427/A problem.
/// </summary>
internal class Solver
{
	...
	{
		int eventsNumber = Input.ReadInt();
		int officersNumber = 0;
		int unsolvedCrimesNumber = 0;
		for (int i = 0; i < eventsNumber; ++i) {
			int eventValue = Input.ReadInt();
			if (eventValue == -1) {
				if (officersNumber == 0) {
					++unsolvedCrimesNumber;
				} else {
					--officersNumber;
				}
			} else if (eventValue > 0) {
				officersNumber += eventValue;
			}
		}
		Output.WriteLine(unsolvedCrimesNumber);
	}
}
