...
/// <summary>
/// Solve https://codeforces.com/problemset/problem/1676/D problem.
/// </summary>
internal class Solver
{
	...
	private static void Solve()
	{
		...
		int[,] grid = new int[rowsCount, columnsCount];
		for (int i = 0; i < rowsCount; ++i) {
			for (int j = 0; j < columnsCount; ++j) {
				grid[i, j] = Input.ReadInt();
			}
		}

		long maxSumOfAllAtackedCellsValues = 0;
		for (int i = 0; i < rowsCount; ++i) {
			for (int j = 0; j < columnsCount; ++j) {
				maxSumOfAllAtackedCellsValues = Math.Max(
					maxSumOfAllAtackedCellsValues,
					CalculateSumOfAllAtackedCellsValues(i, j)
				);
			}
		}
		Output.WriteLine(maxSumOfAllAtackedCellsValues);

		long CalculateSumOfAllAtackedCellsValues(int row, int column)
		{
			long sumOfAllAtackedCellsValues = grid[row, column];
			for (int i = row - 1; i >= 0; --i) {
				int diff = row - i;
				if (column - diff >= 0) {
					sumOfAllAtackedCellsValues += grid[i, column - diff];
				}
				if (column + diff < columnsCount) {
					sumOfAllAtackedCellsValues += grid[i, column + diff];
				}
			}
			...
			return sumOfAllAtackedCellsValues;
		}
	}
}
