...
/// <summary>
/// Solve https://codeforces.com/problemset/problem/1359/B problem.
/// </summary>
internal class Solver
{
	...
	{
		int rowsCount = Input.ReadInt();
		int columnsCount = Input.ReadInt();
		int costOfSinglePaving = Input.ReadInt();
		int costOfDoublePaving = Input.ReadInt();

		bool shouldUseDoublePaving = costOfDoublePaving < costOfSinglePaving * 2;

		int minimalTotalCost = 0;
		int currentWhiteSquaresCount = 0;
		for (int i = 0; i < rowsCount; ++i) {
			string row = Input.ReadString();
			if (row[0] == '.') {
				++currentWhiteSquaresCount;
			}
			for (int j = 1; j < columnsCount; ++j) {
				if (row[j] == '.') {
					++currentWhiteSquaresCount;
				} else if (currentWhiteSquaresCount > 0) {
					minimalTotalCost += CalculateCostOfPaving(currentWhiteSquaresCount);
					currentWhiteSquaresCount = 0;
				}
			}
			if (currentWhiteSquaresCount > 0) {
				minimalTotalCost += CalculateCostOfPaving(currentWhiteSquaresCount);
			}
		}

		Output.WriteLine(minimalTotalCost);

		int CalculateCostOfPaving(int squaresCount)
		{
			if (shouldUseDoublePaving) {
				int doublePavingsCount = Math.DivRem(squaresCount, 2, out int singlePavingsCount);
				return doublePavingsCount * costOfDoublePaving + singlePavingsCount * costOfSinglePaving;
			} else {
				return squaresCount * costOfSinglePaving;
			}
		}
	}
}
