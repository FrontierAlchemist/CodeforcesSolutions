...
/// <summary>
/// Solve https://codeforces.com/problemset/problem/2070/B problem.
/// </summary>
internal class Solver
{
	...
	{
		int commandsCount = Input.ReadInt();
		long currentPoint = Input.ReadInt();
		long observationTime = Input.ReadLong();
		string commands = Input.ReadString();

		long zeroPointReachedCount = 0;
		int currentCommand = 0;
		long loopDuration = 0;
		for (long currentTime = 0; currentTime < observationTime; ++currentTime) {
			if (zeroPointReachedCount > 0) {
				++loopDuration;
			}

			if (commands[currentCommand++] == 'L') {
				--currentPoint;
			} else {
				++currentPoint;
			}
			...
			if (currentPoint == 0) {
				++zeroPointReachedCount;
				currentCommand = 0;

				if (zeroPointReachedCount > 1) {
					long timeLeft = observationTime - currentTime - 1;
					zeroPointReachedCount += timeLeft / loopDuration;
					break;
				}
			}
			
			if (currentCommand == commandsCount) {
				break;
			}
		}


		Output.WriteLine(zeroPointReachedCount);
	}
}
