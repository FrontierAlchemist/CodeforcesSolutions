...
/// <summary>
/// Solve https://codeforces.com/problemset/problem/116/A problem.
/// </summary>
internal class Program
{
	...
	{
		int stopsCount = int.Parse(input.ReadLine());
		int currentPassengersCount = 0;
		int maxPassengersCount = 0;
		for (int i = 0; i < stopsCount; ++i) {
			string[] valuesFromInput = input.ReadLine().Split();
			currentPassengersCount -= int.Parse(valuesFromInput[0]);
			currentPassengersCount += int.Parse(valuesFromInput[1]);
			maxPassengersCount = Math.Max(currentPassengersCount, maxPassengersCount);
		}
		output.WriteLine(maxPassengersCount);
	}
