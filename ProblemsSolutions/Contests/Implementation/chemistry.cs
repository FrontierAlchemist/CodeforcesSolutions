...
/// <summary>
/// Solve https://codeforces.com/problemset/problem/1883/B problem.
/// </summary>
internal class Solver
{
	...
	private static void Solve()
	{
		...
		int charsToRemove = Input.ReadInt();
		string str = Input.ReadString();

		Dictionary<char, int> charsFrequency = new();
		for (int i = 0; i < length; ++i) {
			if (!charsFrequency.ContainsKey(str[i])) {
				charsFrequency.Add(str[i], 0);
			}
			++charsFrequency[str[i]];
		}
		int oddFrequenciesCount = 0;
		int pairsCount = 0;
		foreach (int frequency in charsFrequency.Values) {
			pairsCount += Math.DivRem(frequency, 2, out int remainder);
			oddFrequenciesCount += remainder;
		}

		int difference = oddFrequenciesCount - charsToRemove;
		if (difference > 1) {
			Output.WriteLine("NO");
		} else if (difference == 1 || difference == 0) {
			Output.WriteLine("YES");
		} else {
			charsToRemove -= oddFrequenciesCount;
			int pairsToRemove = charsToRemove / 2;
			if (pairsCount > pairsToRemove) {
				Output.WriteLine("YES");
			} else if (pairsCount == pairsToRemove && (charsToRemove % 2 == 1 && oddFrequenciesCount > 0)) {
				Output.WriteLine("YES");
			} else {
				Output.WriteLine("NO");
			}
