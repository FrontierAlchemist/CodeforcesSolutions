...
/// <summary>
/// Solve https://codeforces.com/problemset/problem/1234/B1 problem.
/// </summary>
internal class Solver
{
	...
	{
		int messagesCount = Input.ReadInt();
		int displayedChatsCount = Input.ReadInt();
		LinkedList<int> displayedMessages = new();
		for (int i = 0; i < messagesCount; ++i) {
			int friendId = Input.ReadInt();
			if (!displayedMessages.Contains(friendId)) {
				if (displayedMessages.Count == displayedChatsCount) {
					displayedMessages.RemoveLast();
				}
				displayedMessages.AddFirst(friendId);
			}
		}
		Output.WriteLine(displayedMessages.Count);
		Output.WriteLine(string.Join(' ', displayedMessages));
	}
}
