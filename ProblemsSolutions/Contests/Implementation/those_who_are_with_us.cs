...
/// <summary>
/// Solve https://codeforces.com/contest/2121/problem/C problem.
/// </summary>
internal class Solver
{
	...
	private static void Solve()
	{
		...
		for (int i = 0; i < rowsCount; ++i) {
			matrix[i] = Input.ReadIntArray(columnsCount);
		}

		(int value, int count) maximum = (0, 0);
		for (int i = 0; i < rowsCount; ++i) {
			for (int j = 0; j < columnsCount; ++j) {
				if (matrix[i][j] > maximum.value) {
					maximum = (matrix[i][j], 1);
				} else if (matrix[i][j] == maximum.value) {
					++maximum.count;
				}
			}
		}

		int[] occurencesOfMaximumPerRow = new int[rowsCount];
		int[] occurencesOfMaximumPerColumn = new int[columnsCount];
		for (int i = 0; i < rowsCount; ++i) {
			for (int j = 0; j < columnsCount; ++j) {
				if (matrix[i][j] == maximum.value) {
					++occurencesOfMaximumPerRow[i];
					++occurencesOfMaximumPerColumn[j];
				}
			}
		}

		bool isMaximumDuplicated = false;
		for (int i = 0; i < rowsCount; ++i) {
			for (int j = 0; j < columnsCount; ++j) {
				int occurencesInCrossing = occurencesOfMaximumPerRow[i] + occurencesOfMaximumPerColumn[j];
				if (matrix[i][j] == maximum.value) {
					--occurencesInCrossing;
				}
				if (occurencesInCrossing == maximum.count) {
					isMaximumDuplicated = true;
					break;
				}
			}
		}

		int minimalMaximalValue = isMaximumDuplicated ? maximum.value - 1 : maximum.value;
		Output.WriteLine(minimalMaximalValue);
	}
}
