...
/// <summary>
/// Solve https://codeforces.com/contest/2123/problem/C problem.
/// </summary>
internal class Solver
{
	...
	private static void Solve()
	{
		...
		char[] requestedSequenceExistPerElement = new char[arraySize];

		int minimumOnPrefix = int.MaxValue;
		for (int i = 0; i < arraySize; ++i) {
			minimumOnPrefix = Math.Min(array[i], minimumOnPrefix);
			if (array[i] == minimumOnPrefix) {
				requestedSequenceExistPerElement[i] = '1';
			}
		}

		int maximumInSuffix = int.MinValue;
		for (int i = arraySize - 1; i >= 0; --i) {
			maximumInSuffix = Math.Max(array[i], maximumInSuffix);
			if (array[i] == maximumInSuffix) {
				requestedSequenceExistPerElement[i] = '1';
			}
		}

		Output.WriteLine(string.Join(' ', requestedSequenceExistPerElement));
	}
}
