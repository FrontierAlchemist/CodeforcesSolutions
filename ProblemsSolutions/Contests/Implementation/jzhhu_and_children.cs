...
/// <summary>
/// Solve https://codeforces.com/problemset/problem/450/A problem.
/// </summary>
internal class Program
{
	...
	{
		int childrensCount = input.GetInt();
		int portionSize = input.GetInt();
		Queue<(int desiredCandiesCount, int number)> childrensQueue = new();
		for (int i = 1; i <= childrensCount; ++i) {
			int desiredCandiesCount = input.GetInt();
			childrensQueue.Enqueue((desiredCandiesCount, i));
		}
		while (childrensQueue.Count > 1) {
			var child = childrensQueue.Dequeue();
			child.desiredCandiesCount -= portionSize;
			if (child.desiredCandiesCount > 0) {
				childrensQueue.Enqueue(child);
			}
		}
		int lastChildrenNumber = childrensQueue.Dequeue().number;
		output.WriteLine(lastChildrenNumber);
	}
