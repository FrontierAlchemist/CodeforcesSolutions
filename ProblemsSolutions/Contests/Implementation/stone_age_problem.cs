...
/// <summary>
/// Solve https://codeforces.com/problemset/problem/1679/B problem.
/// </summary>
internal class Solver
{
	...
	{
		const int ReplaceOneElementQueryNumber = 1;
		const int ReplaceEachElementQueryNumber = 2;

		int arraySize = Input.ReadInt();
		int queriesCount = Input.ReadInt();
		int[] array = Input.ReadIntArray(arraySize);
		int[] changeHistory = new int[arraySize];

		long currentArraySum = 0L;
		foreach (int x in array) {
			currentArraySum += x;
		}

		(int number, int value) lastReplaceEachElementQuery = (0, 0);
		for (int q = 1; q <= queriesCount; ++q) {
			int queryType = Input.ReadInt();
			switch (queryType) {
				case ReplaceOneElementQueryNumber:
					int index = Input.ReadInt() - 1;
					int newValue = Input.ReadInt();

					if (changeHistory[index] < lastReplaceEachElementQuery.number) {
						array[index] = lastReplaceEachElementQuery.value;
					}
					changeHistory[index] = q;

					currentArraySum += newValue - array[index];
					array[index] = newValue;
					break;
				case ReplaceEachElementQueryNumber:
					lastReplaceEachElementQuery = (q, Input.ReadInt());
					currentArraySum = (long)lastReplaceEachElementQuery.value * arraySize;
					break;
			}

			Output.WriteLine(currentArraySum);
		}
	}
}
