...
/// <summary>
/// Solve https://codeforces.com/problemset/problem/1744/C problem.
/// </summary>
internal class Solver
{
	...
	private static void Solve()
	{
		...
		char currentColor = Input.ReadChar();
		string colorsSequence = Input.ReadString();

		int maxIntervalTime = 0;
		if (currentColor != GreenColor) {
			int currentColorLastIndex = -1;
			int greenColorFirstIndex = -1;
			for (int i = 0; i < sequenceLength; ++i) {
				if (colorsSequence[i] == currentColor) {
					if (currentColorLastIndex == -1) {
						currentColorLastIndex = i;
					}
				} else if (colorsSequence[i] == GreenColor) {
					if (currentColorLastIndex != -1) {
						int intervalTime = i - currentColorLastIndex;
						maxIntervalTime = Math.Max(maxIntervalTime, intervalTime);
						currentColorLastIndex = -1;
					}
					if (greenColorFirstIndex == -1) {
						greenColorFirstIndex = i;
					}
				}
			}
			if (currentColorLastIndex != -1) {
				int intervalTime = sequenceLength + greenColorFirstIndex - currentColorLastIndex;
				maxIntervalTime = Math.Max(maxIntervalTime, intervalTime);
			}
		}

		Output.WriteLine(maxIntervalTime);
	}
}
