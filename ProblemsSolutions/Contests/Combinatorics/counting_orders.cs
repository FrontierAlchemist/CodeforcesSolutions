...
/// <summary>
/// Solve https://codeforces.com/problemset/problem/1827/A problem.
/// </summary>
internal class Solver
{
	...
	private static void Solve()
	{
		...
		int array_size = Input.ReadInt();
		int[] array_to_reorder = Input.ReadIntArray(array_size);
		int[] pattern_array = Input.ReadIntArray(array_size);

		Array.Sort(array_to_reorder);
		Array.Sort(pattern_array);

		bool is_reordering_exist = true;
		long ways_to_reordering = 1L;
		int ways_to_put_current_number = 1;
		int index_of_larger_pattern_number = 0;
		for (int i = 0; i < array_size; ++i) {
			--ways_to_put_current_number;
			while (index_of_larger_pattern_number < array_size) {
				if (pattern_array[index_of_larger_pattern_number] < array_to_reorder[i]) {
					++ways_to_put_current_number;
					++index_of_larger_pattern_number;
				} else {
					break;
				}
			}

			if (ways_to_put_current_number <= 0) {
				is_reordering_exist = false;
				break;
			}

			ways_to_reordering = (ways_to_reordering * ways_to_put_current_number) % Module;
		}

		if (!is_reordering_exist) {
			ways_to_reordering = 0L;
		}

		Output.WriteLine(ways_to_reordering);
	}
}
