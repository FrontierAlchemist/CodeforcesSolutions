...
/// <summary>
/// Solve https://codeforces.com/problemset/problem/1840/C problem.
/// </summary>
internal class Solver
{
	...
	private static void Solve()
	{
		...
		int minimalSegmentLength = Input.ReadInt();
		int upperLimit = Input.ReadInt();

		long waysToChooseSegment = 0L;
		int currentSegmentLength = 0;
		for (int i = 0; i < daysCount; ++i) {
			int currentNumber = Input.ReadInt();
			if (currentNumber <= upperLimit) {
				++currentSegmentLength;
			} else if (currentSegmentLength >= minimalSegmentLength) {
				long waysToChooseSegmentWithMinimalLength = currentSegmentLength - minimalSegmentLength + 1L;
				waysToChooseSegment += (waysToChooseSegmentWithMinimalLength + 1L) * waysToChooseSegmentWithMinimalLength / 2L;
		...
		if (currentSegmentLength >= minimalSegmentLength) {
			long waysToChooseSegmentWithMinimalLength = currentSegmentLength - minimalSegmentLength + 1L;
			waysToChooseSegment += (waysToChooseSegmentWithMinimalLength + 1L) * waysToChooseSegmentWithMinimalLength / 2L;
		}

		Output.WriteLine(waysToChooseSegment);
	}
}
