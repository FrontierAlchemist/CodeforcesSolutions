...
/// <summary>
/// Solve https://codeforces.com/problemset/problem/1811/B problem.
/// </summary>
internal class Solver
{
	...
	private struct Point
	{
		...
		public readonly int CalculateRibbonNumber(int matrixSize)
		{
			int halfMatrixSize = matrixSize / 2;
			int a = X;
			int b = Y;
			if (a > halfMatrixSize) {
				a = halfMatrixSize - (a - halfMatrixSize) + 1;
			}
			if (b > halfMatrixSize) {
				b = halfMatrixSize - (b - halfMatrixSize) + 1;
			}
			return Math.Min(a, b);
		}
	}
	...
	private static void Solve()
	{
		...
		int startRibbon = start.CalculateRibbonNumber(matrixSize);
		int endRibbon = end.CalculateRibbonNumber(matrixSize);

		int energyRequired = Math.Abs(startRibbon - endRibbon);
		Output.WriteLine(energyRequired);
	}
}
