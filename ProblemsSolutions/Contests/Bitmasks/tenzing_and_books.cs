...
/// <summary>
/// Solve https://codeforces.com/problemset/problem/1842/B problem.
/// </summary>
internal class Solver
{
	...
	{
		const int StacksCount = 3;

		int stackSize = Input.ReadInt();
		int favoriteNumber = Input.ReadInt();

		int currentKnowledge = 0;
		for (int i = 0; i < StacksCount; ++i) {
			bool isStackBlocked = false;
			for (int j = 0; j < stackSize; ++j) {
				int bookKnowledge = Input.ReadInt();
				if (isStackBlocked) {
					continue;
				}
				int mask = favoriteNumber | bookKnowledge;
				if ((favoriteNumber ^ mask) == 0) {
					currentKnowledge |= bookKnowledge;
				} else {
					isStackBlocked = true;
				}
			}
		...
		Output.WriteLine(currentKnowledge == favoriteNumber ? "YES" : "NO");
	}
}
