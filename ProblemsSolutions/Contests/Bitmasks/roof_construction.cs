...
/// <summary>
/// Solve https://codeforces.com/problemset/problem/1632/B problem.
/// </summary>
internal class Solver
{
	...
	{
		int pilarsCount = Input.ReadInt();
		int minimalCostOfConstruction = 1;
		while ((minimalCostOfConstruction << 1) < pilarsCount) {
			minimalCostOfConstruction <<= 1;
		}

		for (int i = 1; i < minimalCostOfConstruction; ++i) {
			Output.Write($"{i} ");
		}
		Output.Write("0 ");
		for (int i = minimalCostOfConstruction; i < pilarsCount; ++i) {
			Output.Write($"{i} ");
		}
		Output.WriteLine();
	}
