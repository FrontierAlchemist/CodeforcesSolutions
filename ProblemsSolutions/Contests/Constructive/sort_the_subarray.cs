...
/// <summary>
/// Solve https://codeforces.com/problemset/problem/1821/B problem.
/// </summary>
internal class Solver
{
	...
	private static void Solve()
	{
		...
		int[] modifiedArray = Input.ReadIntArray(arraySize);

		int left = 0;
		for (int i = 0; i < arraySize; ++i) {
			if (sourceArray[i] != modifiedArray[i]) {
				left = i;
				break;
			}
		}
		...
		for (int i = right; i < arraySize - 1; ++i) {
			if (modifiedArray[i] <= modifiedArray[i + 1]) {
				++right;
			} else {
				break;
		...
		Output.WriteLine($"{left + 1} {right + 1}");
	}
}
