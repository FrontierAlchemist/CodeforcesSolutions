...
/// <summary>
/// Solve https://codeforces.com/problemset/problem/1859/B problem.
/// </summary>
internal class Solver
{
	...
	{
		int arraysCount = Input.ReadInt();
		int[] firstMinimalElements = new int[arraysCount];
		int[] secondMinimalElements = new int[arraysCount];

		CalculateMinimalsOfNextArray(0);
		int mostMinimalFirstElement = firstMinimalElements[0];
		int mostMinimalSecondElement = secondMinimalElements[0];

		for (int i = 1; i < arraysCount; ++i) {
			CalculateMinimalsOfNextArray(i);
			if (firstMinimalElements[i] < mostMinimalFirstElement) {
				mostMinimalFirstElement = firstMinimalElements[i];
			}
			if (secondMinimalElements[i] < mostMinimalSecondElement) {
				mostMinimalSecondElement = secondMinimalElements[i];
			}
		}

		long maximalBeautyOfArrays = 0L;
		for (int i = 0; i < arraysCount; ++i) {
			maximalBeautyOfArrays += secondMinimalElements[i];
		}

		maximalBeautyOfArrays -= mostMinimalSecondElement;
		maximalBeautyOfArrays += mostMinimalFirstElement;

		Output.WriteLine(maximalBeautyOfArrays);

		void CalculateMinimalsOfNextArray(int arrayIndex)
		{
			int arraySize = Input.ReadInt();

			int firstMinimum = Input.ReadInt();
			int secondMinimum = Input.ReadInt();
			if (secondMinimum < firstMinimum) {
				(firstMinimum, secondMinimum) = (secondMinimum, firstMinimum);
			}
			for (int i = 2; i < arraySize; ++i) {
				int arrayElement = Input.ReadInt();
				if (arrayElement <= firstMinimum) {
					secondMinimum = firstMinimum;
					firstMinimum = arrayElement;
				} else if (arrayElement < secondMinimum) {
					secondMinimum = arrayElement;
				}
			}

			firstMinimalElements[arrayIndex] = firstMinimum;
			secondMinimalElements[arrayIndex] = secondMinimum;
		}
	}
}
