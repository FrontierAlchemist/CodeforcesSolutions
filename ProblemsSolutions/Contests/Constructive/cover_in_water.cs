...
/// <summary>
/// Solve https://codeforces.com/problemset/problem/1900/A problem.
/// </summary>
internal class Solver
{
	...
	{
		int cellsCount = Input.ReadInt();
		string cells = Input.ReadString();
		int emptyCellsInRowCount = 0;
		if (cells[0] == '.') {
			++emptyCellsInRowCount;
		}
		int fillingOperationsCount = 0;
		bool isPossibleToPerfomSecondOperation = false;
		for (int i = 1; i < cellsCount; ++i) {
			if (cells[i] == '#') {
				if (cells[i - 1] == '.') {
					if (emptyCellsInRowCount <= 2) {
						fillingOperationsCount += emptyCellsInRowCount;
						emptyCellsInRowCount = 0;
					} else {
						isPossibleToPerfomSecondOperation = true;
						break;
					}
				}
			} else {
		...
		if (isPossibleToPerfomSecondOperation) {
			fillingOperationsCount = 2;
		}
		Output.WriteLine(fillingOperationsCount);
	}
}
