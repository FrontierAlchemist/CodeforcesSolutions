...
/// <summary>
/// Solve https://codeforces.com/problemset/problem/1433/D problem.
/// </summary>
internal class Solver
{
	...
	{
		int districtsCount = Input.ReadInt();
		int[] districtsOwners = Input.ReadIntArray(districtsCount);

		int firstDisrtict = 0;
		int secondDistrict = -1;
		for (int i = 1; i < districtsCount; ++i) {
			if (districtsOwners[i] != districtsOwners[firstDisrtict]) {
				secondDistrict = i;
			}
		}

		bool isPossibleToConnectDistricts = secondDistrict != -1;
		if (isPossibleToConnectDistricts) {
			Output.WriteLine("YES");
			for (int i = 1; i < districtsCount; ++i) {
				int other = districtsOwners[i] == districtsOwners[firstDisrtict] ? secondDistrict : firstDisrtict;
				Output.WriteLine($"{i + 1} {other + 1}");
			}
		} else {
			Output.WriteLine("NO");
		}
