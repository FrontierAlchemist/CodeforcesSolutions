...
/// <summary>
/// Solve https://codeforces.com/contest/2064/problem/B problem.
/// </summary>
internal class Program
{
	...
	private static void SolveProblem()
	{
		...
		for (int i = 0; i < arraySize; ++i) {
			if (!frequency.TryGetValue(array[i], out int value)) {
				frequency[array[i]] = 0;
			}
			frequency[array[i]] = ++value;
		}

		int maxDistinctSegmentLength = 0;
		int leftPointOfMaxDistinctSegmentLength = 0;
		int rightPointOfMaxDistinctSegmentLength = 0;
		int leftSegmentPoint = -1;
		int rightSegmentPoint = -1;
		for (int i = 0; i < arraySize; ++i) {
			if (frequency[array[i]] == 1) {
				++rightSegmentPoint;
			} else {
				int currentSegmentLength = rightSegmentPoint - leftSegmentPoint;
				if (currentSegmentLength > maxDistinctSegmentLength) {
					leftPointOfMaxDistinctSegmentLength = leftSegmentPoint;
					rightPointOfMaxDistinctSegmentLength = rightSegmentPoint;
					maxDistinctSegmentLength = currentSegmentLength;
				}
				leftSegmentPoint = i;
				rightSegmentPoint = i;
			}
		}
		if (leftSegmentPoint != rightSegmentPoint) {
			int currentSegmentLength = rightSegmentPoint - leftSegmentPoint;
			if (currentSegmentLength > maxDistinctSegmentLength) {
				leftPointOfMaxDistinctSegmentLength = leftSegmentPoint;
				rightPointOfMaxDistinctSegmentLength = rightSegmentPoint;
		...
		if (maxDistinctSegmentLength == 0) {
			output.WriteLine("0");
		} else {
			output.WriteLine($"{leftPointOfMaxDistinctSegmentLength + 2} {rightPointOfMaxDistinctSegmentLength + 1}");
		}
	}
