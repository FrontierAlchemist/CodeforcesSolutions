...
/// <summary>
/// Solve https://codeforces.com/problemset/problem/2022/A problem.
/// </summary>
internal class Solver
{
	...
	{
		int familiesNumber = Input.ReadInt();
		int rowsNumber = Input.ReadInt();
		int[] familiesSize = Input.ReadIntArray(familiesNumber);

		int remainRows = rowsNumber;
		int happyPersonsNumber = 0;
		int separatedPersons = 0;
		for (int i = 0; i < familiesNumber; ++i) {
			int pairsCount = Math.DivRem(familiesSize[i], 2, out int remainPersons);
			
			int takenRows = Math.Min(remainRows, pairsCount);
			remainRows -= takenRows;
			
			happyPersonsNumber += takenRows * 2;
			separatedPersons += remainPersons + (pairsCount - takenRows) * 2;
		}

		if (separatedPersons <= remainRows) {
			happyPersonsNumber += separatedPersons;
		} else {
			happyPersonsNumber += remainRows - (separatedPersons - remainRows);
		}

		Output.WriteLine(happyPersonsNumber);
	}
}
