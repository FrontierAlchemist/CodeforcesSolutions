...
/// <summary>
/// Solve https://codeforces.com/problemset/problem/1704/B problem.
/// </summary>
internal class Solver
{
	...
	private static void Solve()
	{
		...
		int acceptableDifference = Input.ReadInt();

		int number = Input.ReadInt();
		(int l, int r) segment = (number - acceptableDifference, number + acceptableDifference);

		int minimalModificationsCount = 0;
		for (int i = 1; i < arraySize; ++i) {
			number = Input.ReadInt();
			(int l, int r) newSegment = (number - acceptableDifference, number + acceptableDifference);

			segment = (Math.Max(segment.l, newSegment.l), Math.Min(segment.r, newSegment.r));
			if (segment.l > segment.r) {
				segment = newSegment;
				++minimalModificationsCount;
			}
		}

		Output.WriteLine(minimalModificationsCount);
	}
}
