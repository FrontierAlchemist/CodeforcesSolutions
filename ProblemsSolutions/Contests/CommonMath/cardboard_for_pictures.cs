...
/// <summary>
/// Solve https://codeforces.com/problemset/problem/1850/E problem.
/// </summary>
internal class Solver
{
	...
	{
		int picturesCount = Input.ReadInt();
		double usedCardboard = Input.ReadDouble();
		double sizesSum = 0.0;
		double sumOfSquaredSizes = 0.0;
		for (int i = 0; i < picturesCount; ++i) {
			double pictureSize = Input.ReadDouble();
			sizesSum += pictureSize;
			sumOfSquaredSizes += pictureSize * pictureSize;
		}
		double discriminant = sizesSum * sizesSum - picturesCount * (sumOfSquaredSizes - usedCardboard);
		double borderWidth = Math.Round(-sizesSum + Math.Sqrt(discriminant)) / (2.0 * picturesCount);
		Output.WriteLine(borderWidth);
