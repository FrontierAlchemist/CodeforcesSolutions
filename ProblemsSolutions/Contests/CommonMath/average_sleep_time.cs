...
/// <summary>
/// Solve https://codeforces.com/problemset/problem/808/B problem.
/// </summary>
internal class Solver
{
	...
	private static void Solve()
	{
		...
		int checkedSegmentLength = Input.ReadInt();

		long sum = 0L;
		int limit = Math.Min(checkedSegmentLength, daysCount - checkedSegmentLength + 1);
		for (int i = 0; i < daysCount; ++i) {
			long time = Input.ReadLong();
			int factor = Math.Min(limit, Math.Min(i + 1, daysCount - i));
			sum += time * factor;
		}

		double averageTime = (double)sum / (daysCount - checkedSegmentLength + 1);
		Output.WriteLine($"{averageTime:F10}");
	}
}
