...
/// <summary>
/// Solve https://codeforces.com/problemset/problem/1808/B problem.
/// </summary>
internal class Solver
{
	...
	private static void Solve()
	{
		...
		int numbersOnCardCount = Input.ReadInt();
		long[][] numbersOnCard = new long[numbersOnCardCount][];
		for (int i = 0; i < numbersOnCardCount; ++i) {
			numbersOnCard[i] = new long[playersCount];
		}
		for (int j = 0; j < playersCount; ++j) {
			for (int i = 0; i < numbersOnCardCount; ++i) {
				numbersOnCard[i][j] = Input.ReadLong();
			}
		}

		long totalPotSize = 0L;
		for (int i = 0; i < numbersOnCardCount; ++i) {
			Array.Sort(numbersOnCard[i]);
			for (int j = 0; j < playersCount; ++j) {
				int count = j - playersCount + j + 1;
				totalPotSize += count * numbersOnCard[i][j];
			}
		}

		Output.WriteLine(totalPotSize);
	}
}
