...
/// <summary>
/// Solve https://codeforces.com/problemset/problem/1742/E problem.
/// </summary>
internal class Solver
{
	...
	{
		int stepsCount = Input.ReadInt();
		int queriesCount = Input.ReadInt();
		int[] steps = Input.ReadIntArray(stepsCount);
		(int height, int index)[] queries = new (int, int)[queriesCount];
		for (int i = 0; i < queriesCount; ++i) {
			queries[i] = (Input.ReadInt(), i);
		}
		Array.Sort(queries);

		long[] responses = new long[queriesCount];
		long currentHeight = 0L;
		int currentQueryIndex = 0;
		for (int i = 0; i < stepsCount; ++i) {
			while (currentQueryIndex < queriesCount && queries[currentQueryIndex].height < steps[i]) {
				responses[queries[currentQueryIndex++].index] = currentHeight;
			}
			if (currentQueryIndex >= queriesCount) {
				break;
			}
			currentHeight += steps[i];
		}
		while (currentQueryIndex < queriesCount) {
			responses[queries[currentQueryIndex++].index] = currentHeight;
		}

		Output.WriteLine(string.Join(' ', responses));
	}
}
