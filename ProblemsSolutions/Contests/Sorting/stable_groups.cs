...
/// <summary>
/// Solve https://codeforces.com/problemset/problem/1539/C problem.
/// </summary>
internal class Solver
{
	...
	{
		int studentsCount = Input.ReadInt();
		long extraPullSize = Input.ReadLong();
		long limit = Input.ReadLong();
		long[] levels = Input.ReadLongArray(studentsCount);

		Array.Sort(levels);

		int groupsCount = 1;
		long[] differncesFactors = new long[studentsCount - 1];
		for (int i = 0; i < studentsCount - 1; ++i) {
			long factorOfDifference = Math.DivRem(levels[i + 1] - levels[i], limit, out long remainder);
			differncesFactors[i] = remainder == 0 && factorOfDifference > 0 ? factorOfDifference - 1 : factorOfDifference;
			if (differncesFactors[i] > 0L) {
				++groupsCount;
			}
		}

		Array.Sort(differncesFactors);

		for (int i = 0; i < studentsCount - 1; ++i) {
			if (differncesFactors[i] == 0) {
				continue;
			}
			if (differncesFactors[i] <= extraPullSize) {
				extraPullSize -= differncesFactors[i];
				--groupsCount;
			} else {
				break;
			}
