...
/// <summary>
/// Solve https://codeforces.com/contest/2126/problem/B problem.
/// </summary>
internal class Solver
{
	...
	private static void Solve()
	{
		...
		int requiredGoodDays = Input.ReadInt();
		int[] weatherPerDay = Input.ReadIntArray(daysCount);
		int goodWeatherConsecutiveDays = 0;
		int climbedPeaks = 0;
		for (int i = 0; i < daysCount; ++i) {
			if (weatherPerDay[i] == 0) {
				++goodWeatherConsecutiveDays;
			} else {
				goodWeatherConsecutiveDays = 0;
			}
			if (goodWeatherConsecutiveDays == requiredGoodDays) {
				++climbedPeaks;
				goodWeatherConsecutiveDays = -1;
			}
		}
		Output.WriteLine(climbedPeaks);
	}
}
