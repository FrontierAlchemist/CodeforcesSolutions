...
/// <summary>
/// Solve https://codeforces.com/problemset/problem/1841/B problem.
/// </summary>
internal class Solver
{
	...
	private static void Solve()
	{
		...
		int firstElement = Input.ReadInt();
		int lastInsertedElement = firstElement;

		bool[] isElementInserted = new bool[queriesCount];
		isElementInserted[0] = true;

		bool isArraySorted = true;

		for (int i = 1; i < queriesCount; ++i) {
			int currentElement = Input.ReadInt();
			if (isArraySorted) {
				if (currentElement >= lastInsertedElement) {
					isElementInserted[i] = true;
				...
				if (currentElement >= lastInsertedElement && currentElement <= firstElement) {
					isElementInserted[i] = true;
					lastInsertedElement = currentElement;
				}
			}
		...
		for (int i = 0; i < queriesCount; ++i) {
			Output.Write(isElementInserted[i] ? 1 : 0);
		}
		Output.WriteLine();
	}
}
