...
/// <summary>
/// Solve https://codeforces.com/problemset/problem/1907/B problem.
/// </summary>
internal class Solver
{
	...
	private static void Solve()
	{
		...
		int lowerCaseRemovingOperations = 0;
		int upperCaseRemovingOperations = 0;
		for (int i = sourceString.Length - 1; i >= 0; --i) {
			switch (sourceString[i]) {
				case 'b':
					++lowerCaseRemovingOperations;
					break;
				case 'B':
					++upperCaseRemovingOperations;
					break;
				default:
					if (char.IsLower(sourceString[i])) {
						if (lowerCaseRemovingOperations > 0) {
							--lowerCaseRemovingOperations;
						} else {
							stack.Push(sourceString[i]);
						}
					} else {
						if (upperCaseRemovingOperations > 0) {
							--upperCaseRemovingOperations;
						} else {
							stack.Push(sourceString[i]);
						}
