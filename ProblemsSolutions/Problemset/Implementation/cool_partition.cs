...
/// <summary>
/// Solve https://codeforces.com/contest/2117/problem/C problem.
/// </summary>
internal class Solver
{
	...
	private static void Solve()
	{
		...
		Stack<HashSet<int>> stackOfPreviousSegmentsDistinctNumbersSets = new();
		HashSet<int> currentSegmentDistinctNumbersSet = new();
		for (int i = arraySize - 1; i >= 0; --i) {
			if (currentSegmentDistinctNumbersSet.Contains(array[i])) {
				stackOfPreviousSegmentsDistinctNumbersSets.Push(currentSegmentDistinctNumbersSet);
				currentSegmentDistinctNumbersSet = new() { array[i] };
			} else {
				while (stackOfPreviousSegmentsDistinctNumbersSets.Count > 0) {
					if (
						stackOfPreviousSegmentsDistinctNumbersSets.TryPeek(out var previousSet) &&
						previousSet.Contains(array[i])
					) {
						break;
					} else {
						currentSegmentDistinctNumbersSet = stackOfPreviousSegmentsDistinctNumbersSets.Pop();
					}
				}
				currentSegmentDistinctNumbersSet.Add(array[i]);
			}
		}

		Output.WriteLine(stackOfPreviousSegmentsDistinctNumbersSets.Count + 1);
	}
}
