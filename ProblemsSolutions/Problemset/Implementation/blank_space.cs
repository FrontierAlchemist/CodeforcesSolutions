...
/// <summary>
/// Solve https://codeforces.com/problemset/problem/1829/B problem.
/// </summary>
internal class Solver
{
	...
	private static void Solve()
	{
		...
		int maxSpaceSize = 0;
		int currentSpaceSize = array[0] == 0 ? 1 : 0;
		for (int i = 1; i < arraySize; ++i) {
			if (array[i] == 0) {
				++currentSpaceSize;
			} else if (array[i - 1] == 0 && array[i] == 1) {
				maxSpaceSize = Math.Max(maxSpaceSize, currentSpaceSize);
				currentSpaceSize = 0;
			}
		}
		maxSpaceSize = Math.Max(maxSpaceSize, currentSpaceSize);
		Output.WriteLine(maxSpaceSize);
	}
}
