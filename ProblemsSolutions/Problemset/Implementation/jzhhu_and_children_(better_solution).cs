...
internal class Program
{
	...
	private static void SolveProblem()
	{
		...
		int maxCyclesNumber = int.MinValue;
		int lastChildrenNumber = -1;
		for (int i = 1; i <= childrensCount; ++i) {
			int desiredCandiesCount = input.GetInt();
			int cyclesNumber = Math.DivRem(desiredCandiesCount, portionSize, out int remainder);
			if (remainder > 0) {
				++cyclesNumber;
			}
			if (cyclesNumber >= maxCyclesNumber) {
				maxCyclesNumber = cyclesNumber;
				lastChildrenNumber = i;
			}
		}
