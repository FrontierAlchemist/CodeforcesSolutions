...
/// <summary>
/// Solve https://codeforces.com/contest/1915/problem/B problem.
/// </summary>
internal class Solver
{
	...
	private static void Solve()
	{
		...
		for (int i = 0; i < RowsCount; ++i) {
			bool[] isRowContainsLetter = new bool[ColumnsCount];

			string row = Input.ReadString();
			for (int j = 0; j < ColumnsCount; ++j) {
				if (row[j] != '?') {
					isRowContainsLetter[row[j] - 'A'] = true;
				}
			}

			for (int j = 0; j < ColumnsCount; ++j) {
				if (!isRowContainsLetter[j]) {
					replacedLetter = (char)('A' + j);
				}
			}
		}

		Output.WriteLine(replacedLetter);
	}
}
