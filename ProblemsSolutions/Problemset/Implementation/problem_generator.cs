...
/// <summary>
/// Solve https://codeforces.com/contest/1980/problem/A problem.
/// </summary>
internal class Solver
{
	...
	{
		const int DifficultiesNumber = 7;

		int problemsCount = Input.ReadInt();
		int roundsCount = Input.ReadInt();
		string problemsPull = Input.ReadString();

		int[] problemsPerDifficulty = new int[DifficultiesNumber];
		for (int i = 0; i < problemsCount; ++i) {
			int difficulty = problemsPull[i] - 'A';
			++problemsPerDifficulty[difficulty];
		}

		int additionalProblemsCount = 0;
		for (int i = 0; i < DifficultiesNumber; ++i) {
			additionalProblemsCount += Math.Max(roundsCount - problemsPerDifficulty[i], 0);
		}
		Output.WriteLine(additionalProblemsCount);
	}
}
