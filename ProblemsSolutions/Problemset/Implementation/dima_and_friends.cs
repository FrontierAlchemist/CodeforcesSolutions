...
/// <summary>
/// Solve https://codeforces.com/problemset/problem/272/A problem.
/// </summary>
internal class Solver
{
	...
	{
		int friendsCount = Input.ReadInt();
		int fingersCount = 0;
		for (int i = 0; i < friendsCount; ++i) {
			fingersCount += Input.ReadInt();
		}
		int waysCount = 0;
		for (int i = 1; i <= 5; ++i) {
			if ((fingersCount + i) % (friendsCount + 1) != 1) {
				++waysCount;
			}
		}
		Output.WriteLine(waysCount);
	}
}
