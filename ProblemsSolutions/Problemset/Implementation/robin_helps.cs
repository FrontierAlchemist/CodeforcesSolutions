...
/// <summary>
/// Solve https://codeforces.com/problemset/problem/2014/A problem.
/// </summary>
internal class Solver
{
	...
	{
		int peoplesNumber = Input.ReadInt();
		int robberyTreshold = Input.ReadInt();

		int robinBalance = 0;
		int donationsCount = 0;
		for (int i = 0; i < peoplesNumber; ++i) {
			int goldNumber = Input.ReadInt();
			if (goldNumber >= robberyTreshold) {
				robinBalance += goldNumber;
			} else if (goldNumber == 0 && robinBalance > 0) {
				--robinBalance;
				++donationsCount;
			}
		}

		Output.WriteLine(donationsCount);
	}
}
