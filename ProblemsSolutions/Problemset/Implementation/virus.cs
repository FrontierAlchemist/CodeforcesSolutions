...
/// <summary>
/// Solve https://codeforces.com/problemset/problem/1704/C problem.
/// </summary>
internal class Solver
{
	...
	{
		int housesCount = Input.ReadInt();
		int infectedHousesCount = Input.ReadInt();
		int[] infectedHouses = Input.ReadIntArray(infectedHousesCount);
		Array.Sort(infectedHouses);

		List<int> uninfectedHousesSegmentsLengthes = new();
		
		if (infectedHousesCount == 1) {
			uninfectedHousesSegmentsLengthes.Add(housesCount - 1);
		} else {
			for (int i = 1; i < infectedHousesCount; ++i) {
				uninfectedHousesSegmentsLengthes.Add(infectedHouses[i] - infectedHouses[i - 1] - 1);
			}
			uninfectedHousesSegmentsLengthes.Add(infectedHouses[0] - 1 + housesCount - infectedHouses[^1]);
		}

		uninfectedHousesSegmentsLengthes.Sort();

		int savedHousesCount = 0;
		int lostHouses = 0;
		for (int i = uninfectedHousesSegmentsLengthes.Count - 1; i >= 0; --i) {
			uninfectedHousesSegmentsLengthes[i] -= lostHouses;
			if (uninfectedHousesSegmentsLengthes[i] <= 0) {
				break;
			}
			++savedHousesCount;
			uninfectedHousesSegmentsLengthes[i] -= 2;
			if (uninfectedHousesSegmentsLengthes[i] <= 0) {
				break;
			}
			savedHousesCount += uninfectedHousesSegmentsLengthes[i];
			lostHouses += 4;
		}

		int finalyInfectedHouses = housesCount - savedHousesCount;
		Output.WriteLine(finalyInfectedHouses);
	}
}
