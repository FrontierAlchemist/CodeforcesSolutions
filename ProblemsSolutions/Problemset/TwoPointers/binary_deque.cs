...
/// <summary>
/// Solve https://codeforces.com/problemset/problem/1692/E problem.
/// </summary>
internal class Solver
{
	...
	private static void Solve()
	{
		...
		int minimumOperationsToReachTargetSum = -1;
		if (totalSum == targetSum) {
			minimumOperationsToReachTargetSum = 0;
		} else if (totalSum > targetSum) {
			int onesToRemove = totalSum - targetSum;
			int removedOnes = 0;
			int prefixEnd = 0;
			for (int i = 0; i < arraySize; ++i) {
				if (array[i] == 1) {
				...
				if (removedOnes == onesToRemove) {
					prefixEnd = i;
					break;
				}
			}

			int operationsCount = prefixEnd + 1;
			minimumOperationsToReachTargetSum = operationsCount;

			int suffixEnd = arraySize;
			while (prefixEnd >= 0 && suffixEnd >= 0) {
				if (array[prefixEnd] == 1) {
					--operationsCount;
					--prefixEnd;
					--removedOnes;
					while (prefixEnd >= 0 && array[prefixEnd] == 0) {
						--operationsCount;
						--prefixEnd;
					}
					while (--suffixEnd >= 0) {
						++operationsCount;
						if (array[suffixEnd] == 1) {
							++removedOnes;
							break;
						}
					}
				...
				if (removedOnes == onesToRemove) {
					minimumOperationsToReachTargetSum = Math.Min(operationsCount, minimumOperationsToReachTargetSum);
				}
			}
		}

		Output.WriteLine(minimumOperationsToReachTargetSum);
	}
}
