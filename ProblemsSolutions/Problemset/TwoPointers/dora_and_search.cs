...
/// <summary>
/// Solve https://codeforces.com/contest/1793/problem/C problem.
/// </summary>
internal class Solver
{
	...
	{
		int permutationLength = Input.ReadInt();
		int[] permutation = Input.ReadIntArray(permutationLength);

		int currentMinimum = 1;
		int currentMaximum = permutationLength;
		int leftPointer = 0;
		int rightPointer = permutationLength - 1;

		while (leftPointer < rightPointer) {
			if (permutation[leftPointer] == currentMinimum) {
				++leftPointer;
				++currentMinimum;
			} else if (permutation[leftPointer] == currentMaximum) {
				++leftPointer;
				--currentMaximum;
			} else if (permutation[rightPointer] == currentMinimum) {
				--rightPointer;
				++currentMinimum;
			} else if (permutation[rightPointer] == currentMaximum) {
				--rightPointer;
				--currentMaximum;
			} else {
				break;
		...
		bool isRequestedSegmentExist = leftPointer < rightPointer;
		Output.WriteLine(isRequestedSegmentExist ? $"{leftPointer + 1} {rightPointer + 1}" : "-1");
	}
}
