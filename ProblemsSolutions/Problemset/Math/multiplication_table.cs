...
/// <summary>
/// Solve https://codeforces.com/problemset/problem/577/A problem.
/// </summary>
internal class Program
{
	...
	{
		long numbersCount = input.GetLong();
		long searchedNumber = input.GetLong();
		int occurencesOfSearchedNumber = 0;
		long border = Math.Min(numbersCount, searchedNumber);
		for (long i = 1; i <= border; ++i) {
			long otherMultiplier = Math.DivRem(searchedNumber, i, out long remainder);
			if (otherMultiplier <= numbersCount && remainder == 0) {
				++occurencesOfSearchedNumber;
			}
		}
		output.WriteLine(occurencesOfSearchedNumber);
	}
