...
/// <summary>
/// Solve https://codeforces.com/problemset/problem/2130/A problem.
/// </summary>
internal class Solver
{
	...
	private static void Solve()
	{
		...
		int sequenceSize = Input.ReadInt();
		for (int i = 0; i < sequenceSize; ++i) {
			++frequencies[Input.ReadInt()];
		}

		int zeroOnesPairs = Math.Min(frequencies[0], frequencies[1]);
		int aloneZeroes = Math.Max(frequencies[0] - zeroOnesPairs, 0);
		int aloneOnes = Math.Max(frequencies[1] - zeroOnesPairs, 0);
		int bestScore = zeroOnesPairs * 2 + aloneZeroes + aloneOnes;
		for (int i = 2; i <= MaxValue; ++i) {
			bestScore += frequencies[i] * i;
		}

		Output.WriteLine(bestScore);
	}
}
