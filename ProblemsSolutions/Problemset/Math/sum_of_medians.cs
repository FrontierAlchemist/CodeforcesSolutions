...
/// <summary>
/// Solve https://codeforces.com/problemset/problem/1440/B problem.
/// </summary>
internal class Solver
{
	...
	private static void Solve()
	{
		...
		int partsCount = Input.ReadInt();

		int arraySize = partSize * partsCount;
		int[] array = Input.ReadIntArray(arraySize);
		int medianPosition = Math.DivRem(partSize, 2, out int remadiner) + remadiner - 1;

		int startPosition = medianPosition * partsCount;
		int stepLength = partSize - medianPosition;
		long mediansSum = 0L;
		for (int i = startPosition; i < arraySize; i += stepLength) {
			mediansSum += array[i];
		}

		Output.WriteLine(mediansSum);
	}
}
