...
/// <summary>
/// Solve https://codeforces.com/problemset/problem/1878/C problem.
/// </summary>
internal class Solver
{
	...
	{
		long availableElementsCount = Input.ReadLong();
		long targetElementsCount = Input.ReadLong();
		long targetSum = Input.ReadLong();

		long minSum = targetElementsCount * (targetElementsCount + 1L) / 2L;
		
		long leftElementsCount = availableElementsCount - targetElementsCount;
		long leftSum = leftElementsCount * (leftElementsCount + 1L) / 2L;
		long totalSum = availableElementsCount * (availableElementsCount + 1L) / 2L;
		long maxSum = totalSum - leftSum;

		bool isTargetSubsetExist = minSum <= targetSum && targetSum <= maxSum;
		Output.WriteLine(isTargetSubsetExist ? "YES" : "NO");
	}
}
