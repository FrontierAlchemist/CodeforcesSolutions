...
/// <summary>
/// Solve https://codeforces.com/contest/2091/problem/E problem.
/// </summary>
internal class Solver
{
	...
	private const long MaxNumber = 10000000L;

	private readonly static bool[] isPrimeNumber = new bool[MaxNumber + 1];

	private static void MarkPrimeNumbers()
	{
		...
		for (long i = 2; i <= MaxNumber; ++i) {
			long nextCompositeNumber = i * i;
			if (nextCompositeNumber > MaxNumber) {
				break;
			}
			if (!isPrimeNumber[i]) {
				continue;
			}
			while (nextCompositeNumber <= MaxNumber) {
				isPrimeNumber[nextCompositeNumber] = false;
				nextCompositeNumber += i;
			}
		}
	}
	...
	{
		int n = Input.ReadInt();
		long possiblePairsCount = 0;
		for (int i = 2; i <= n; ++i) {
			if (isPrimeNumber[i]) {
				possiblePairsCount += n / i;
			}
		}
		Output.WriteLine(possiblePairsCount);
	}
}
