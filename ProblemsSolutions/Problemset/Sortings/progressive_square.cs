...
/// <summary>
/// Solve https://codeforces.com/problemset/problem/1955/B problem.
/// </summary>
internal class Solver
{
	...
	private static void Solve()
	{
		...
		int incrementationPerRow = Input.ReadInt();
		int incrementationPerColumn = Input.ReadInt();
		int arraySize = matrixLength * matrixLength;
		int[] sourceArray = Input.ReadIntArray(arraySize);

		Array.Sort(sourceArray);

		int[] createdArray = new int[arraySize];
		createdArray[0] = sourceArray[0];

		for (int j = 1; j < matrixLength; ++j) {
			createdArray[j] = createdArray[j - 1] + incrementationPerColumn;
		}
		for (int i = 1; i < matrixLength; ++i) {
			int currentIndex = matrixLength * i;
			createdArray[currentIndex] = createdArray[currentIndex - matrixLength] + incrementationPerRow;
			for (int j = 1; j < matrixLength; ++j) {
				++currentIndex;
				createdArray[currentIndex] = createdArray[currentIndex - 1] + incrementationPerColumn;
			}
		}

		Array.Sort(createdArray);

		bool isSourceArrayCorrect = true;
		for (int i = 0; i < arraySize; ++i) {
			if (sourceArray[i] != createdArray[i]) {
				isSourceArrayCorrect = false;
				break;
			}
		}
		Output.WriteLine(isSourceArrayCorrect ? "YES" : "NO");
	}
}
