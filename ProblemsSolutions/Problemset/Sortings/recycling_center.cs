...
/// <summary>
/// Solve https://codeforces.com/contest/2128/problem/A problem.
/// </summary>
internal class Solver
{
	...
	{
		int trashBagsCount = Input.ReadInt();
		int treshold = Input.ReadInt();
		int[] secondsToPassThreshold = new int[trashBagsCount];
		for (int i = 0; i < trashBagsCount; ++i) {
			long trashBagWeight = Input.ReadLong();
			while (trashBagWeight <= treshold) {
				++secondsToPassThreshold[i];
				trashBagWeight <<= 1;
			}
		}

		Array.Sort(secondsToPassThreshold);

		int minimalTotalCost = 0;
		int secondsPassed = 0;
		for (int i = 0; i < trashBagsCount; ++i) {
			if (secondsToPassThreshold[i] <= secondsPassed) {
				++minimalTotalCost;
			} else {
				++secondsPassed;
			}
