...
/// <summary>
/// Solve https://codeforces.com/contest/2117/problem/A problem.
/// </summary>
internal class Solver
{
	...
	{
		int doorsCount = Input.ReadInt();
		int buttonEffectTime = Input.ReadInt();
		int firstClosedDoorIndex = -1;
		int lastClosedDoorIndex = -1;
		for (int i = 0; i < doorsCount; ++i) {
			int c = Input.ReadInt();
			if (c == 1) {
				if (firstClosedDoorIndex == -1) {
					firstClosedDoorIndex = i;
					lastClosedDoorIndex = i;
				} else {
					lastClosedDoorIndex = i;
				}
			}
		...
		int requiredTimeToButtonEffect = lastClosedDoorIndex - firstClosedDoorIndex + 1;
		bool isPossibleToCloseAllDoors = firstClosedDoorIndex == -1 || buttonEffectTime >= requiredTimeToButtonEffect;
		Output.WriteLine(isPossibleToCloseAllDoors ? "YES" : "NO");
	}
}
