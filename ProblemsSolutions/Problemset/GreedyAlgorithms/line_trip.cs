...
/// <summary>
/// Solve https://codeforces.com/problemset/problem/1901/A problem.
/// </summary>
internal class Solver
{
	...
	{
		int gasStationsCount = Input.ReadInt();
		int endPoint = Input.ReadInt();
		int[] gasStations = Input.ReadIntArray(gasStationsCount);

		int minimalTankVolume = gasStations[0];
		for (int i = 1; i < gasStationsCount; ++i) {
			minimalTankVolume = Math.Max(minimalTankVolume, gasStations[i] - gasStations[i - 1]);
		}
		minimalTankVolume = Math.Max(minimalTankVolume, (endPoint - gasStations[gasStationsCount - 1]) * 2);

		Output.WriteLine(minimalTankVolume);
	}
}
