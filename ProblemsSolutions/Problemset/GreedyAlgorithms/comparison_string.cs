...
/// <summary>
/// Solve https://codeforces.com/problemset/problem/1837/B problem.
/// </summary>
internal class Solver
{
	...
	private static void Solve()
	{
		...
		string sequence = Input.ReadString();
		int longestEqualSubsequenceLength = 0;
		int currentEqualSubsequenceLength = 1;
		for (int i = 1; i < sequenceLength; ++i) {
			if (sequence[i] != sequence[i - 1]) {
				longestEqualSubsequenceLength = Math.Max(longestEqualSubsequenceLength, currentEqualSubsequenceLength);
				currentEqualSubsequenceLength = 1;
			} else {
				++currentEqualSubsequenceLength;
			}
		}
		longestEqualSubsequenceLength = Math.Max(longestEqualSubsequenceLength, currentEqualSubsequenceLength);
		Output.WriteLine(longestEqualSubsequenceLength + 1);
	}
}
