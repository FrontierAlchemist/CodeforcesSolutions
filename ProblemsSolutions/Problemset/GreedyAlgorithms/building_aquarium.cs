...
/// <summary>
/// Solve https://codeforces.com/problemset/problem/1873/E problem.
/// </summary>
internal class Solver
{
	...
	private static void Solve()
	{
		...
		int volumeLimit = Input.ReadInt();
		int[] columnsHeights = Input.ReadIntArray(columnsCount);

		Array.Sort(columnsHeights);

		long currentVolume = 0;
		int maximalHeight = columnsHeights[0];
		int usedColumns = 1;
		for (; usedColumns < columnsCount; ++usedColumns) {
			if (columnsHeights[usedColumns] == columnsHeights[usedColumns - 1]) {
				continue;
			}
			long newVolume = currentVolume + (long)(columnsHeights[usedColumns] - columnsHeights[usedColumns - 1]) * usedColumns;
			if (newVolume > volumeLimit) {
				break;
			}
			currentVolume = newVolume;
			maximalHeight = columnsHeights[usedColumns];
		}

		long volumeLeft = volumeLimit - currentVolume;
		if (volumeLeft > 0) {
			maximalHeight += (int)(volumeLeft / usedColumns);
		}
		Output.WriteLine(maximalHeight);
	}
}
