...
/// <summary>
/// Solve https://codeforces.com/contest/2123/problem/B problem.
/// </summary>
internal class Solver
{
	...
	{
		int playersCount = Input.ReadInt();
		int targetPlayerNumber = Input.ReadInt() - 1;
		int lastPlayersCount = Input.ReadInt();
		int[] playersPower = Input.ReadIntArray(playersCount);
		int weakerPlayersCount = 0;
		int strongerPlayersCount = 0;
		for (int i = 0; i < playersCount; ++i) {
			if (i == targetPlayerNumber) {
				continue;
			}
			if (playersPower[i] <= playersPower[targetPlayerNumber]) {
				++weakerPlayersCount;
			} else {
				++strongerPlayersCount;
			}
		}
		int maximalEliminatedPlayersCount = weakerPlayersCount + (strongerPlayersCount == 0 ? 0 : strongerPlayersCount - 1);
		bool isPossibleToTargetPlayerRemain = maximalEliminatedPlayersCount + lastPlayersCount >= playersCount;
		Output.WriteLine(isPossibleToTargetPlayerRemain ? "YES" : "NO");
	}
}
