...
/// <summary>
/// Solve https://codeforces.com/problemset/problem/1665/B problem.
/// </summary>
internal class Solver
{
	...
	private static void Solve()
	{
		...
		int maxOccurrency = 0;
		Dictionary<int, int> occurrencesFrequency = new();
		for (int i = 0; i < arraySize; ++i) {
			int number = Input.ReadInt();
			if (!occurrencesFrequency.TryGetValue(number, out int occurrency)) {
				occurrencesFrequency[number] = 0;
				occurrency = 0;
			}
			maxOccurrency = Math.Max(maxOccurrency, occurrency + 1);
			++occurrencesFrequency[number];
		}

		int minimalModificationOperationsCount = 0;
		while (maxOccurrency < arraySize) {
			++minimalModificationOperationsCount;
			int newMaxOccurency = Math.Min(arraySize, maxOccurrency * 2);
			int createdClones = newMaxOccurency - maxOccurrency;
			maxOccurrency = newMaxOccurency;
			minimalModificationOperationsCount += createdClones;
		}
		Output.WriteLine(minimalModificationOperationsCount);
	}
}
