...
/// <summary>
/// Solve https://codeforces.com/problemset/problem/1985/C problem.
/// </summary>
internal class Program
{
	...
	private static void SolveProblem()
	{
		...
		var array = input.GetIntArray(arraySize);
		long currentSum = 0L;
		long currentMainElement = array[0];
		int goodPrefixesNumber = currentSum == currentMainElement ? 1 : 0;
		for (int i = 1; i < arraySize; ++i) {
			if (array[i] > currentMainElement) {
				currentSum += currentMainElement;
				currentMainElement = array[i];
			} else {
				currentSum += array[i];
			}
			if (currentSum == currentMainElement) {
				++goodPrefixesNumber;
			}
		}
		output.WriteLine(goodPrefixesNumber);
	}
