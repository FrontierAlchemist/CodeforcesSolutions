...
/// <summary>
/// Solve https://codeforces.com/problemset/problem/1696/B problem.
/// </summary>
internal class Solver
{
	...
	private static void Solve()
	{
		...
		if (firstNonZeroNumberIndex == arraySize) {
			Output.WriteLine(minimalOperationsCount);
			return;
		}

		int lastNonZeroNumberIndex = arraySize - 1;
		for (int i = arraySize - 1; i >= 0; --i) {
			if (array[i] == 0) {
				--lastNonZeroNumberIndex;
			} else {
				break;
			}
		}
		bool isZeroExistInSegement = false;
		for (int i = firstNonZeroNumberIndex + 1; i < lastNonZeroNumberIndex; ++i) {
			if (array[i] == 0) {
				isZeroExistInSegement = true;
				break;
			}
		}

		minimalOperationsCount = isZeroExistInSegement ? 2 : 1;
		Output.WriteLine(minimalOperationsCount);
	}
}
