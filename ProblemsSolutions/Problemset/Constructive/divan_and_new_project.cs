...
/// <summary>
/// Solve https://codeforces.com/problemset/problem/1614/B problem.
/// </summary>
internal class Solver
{
	...
	{
		int minorBuildingsCount = Input.ReadInt();
		(int visitingCounts, int index)[] minorBuildingsVisitingCount = new (int, int)[minorBuildingsCount];
		for (int i = 0; i < minorBuildingsCount; ++i) {
			minorBuildingsVisitingCount[i].visitingCounts = Input.ReadInt();
			minorBuildingsVisitingCount[i].index = i + 1;
		}
		Array.Sort(minorBuildingsVisitingCount);

		long minimalTimeForWalking = 0L;
		int[] buildingsCoordinations = new int[minorBuildingsCount + 1];
		int currentCoordinate = 1;
		for (int i = 1; i <= minorBuildingsCount; ++i) {
			(int visitingCounts, int index) = minorBuildingsVisitingCount[minorBuildingsCount - i];
			buildingsCoordinations[index] = currentCoordinate;
			minimalTimeForWalking += (long)Math.Abs(currentCoordinate) * visitingCounts * 2;
			currentCoordinate = currentCoordinate > 0 ? -currentCoordinate : (-currentCoordinate) + 1;
		}

		Output.WriteLine(minimalTimeForWalking);
		Output.WriteLine(string.Join(' ', buildingsCoordinations));
	}
}
