...
/// <summary>
/// Solve https://codeforces.com/problemset/problem/1511/C problem.
/// </summary>
internal class Solver
{
	...
	{
		const int ColorsCount = 50;
		int[] cardsIndexPerColors = new int[ColorsCount];

		int cardsCount = Input.ReadInt();
		int queriesCount = Input.ReadInt();
		for (int i = 0; i < cardsCount; ++i) {
			int cardColor = Input.ReadInt() - 1;
			if (cardsIndexPerColors[cardColor] == 0) {
				cardsIndexPerColors[cardColor] = i + 1;
			}
		}

		for (int q = 0; q < queriesCount; ++q) {
			int color = Input.ReadInt() - 1;
			int position = cardsIndexPerColors[color];
			Output.Write($"{position} ");
			for (int i = 0; i < ColorsCount; ++i) {
				if (cardsIndexPerColors[i] < position) {
					++cardsIndexPerColors[i];
				}
			}
			cardsIndexPerColors[color] = 1;
		}
		Output.WriteLine();
	}
}
