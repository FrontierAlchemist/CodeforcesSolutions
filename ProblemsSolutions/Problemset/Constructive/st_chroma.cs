...
/// <summary>
/// Solve https://codeforces.com/contest/2106/problem/B problem.
/// </summary>
internal class Solver
{
	...
	private static void Solve()
	{
		...
		int targetColor = Input.ReadInt();
		int[] permutation = new int[permutationLength];
		for (int i = 0; i < permutationLength; ++i) {
			permutation[i] = i;
		}
		if (targetColor < permutationLength) {
			(permutation[targetColor], permutation[permutationLength - 1]) = (permutation[permutationLength - 1], permutation[targetColor]);
		}
		Output.WriteLine(string.Join(' ', permutation));
	}
}
