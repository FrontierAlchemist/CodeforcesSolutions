...
/// <summary>
/// Solve https://codeforces.com/contest/2121/problem/A problem.
/// </summary>
internal class Solver
{
	...
	private static void Solve()
	{
		...
		int startPoint = Input.ReadInt();
		int leftPoint = Input.ReadInt();

		int minimalStepsCount;
		if (pointsCount == 1) {
			minimalStepsCount = Math.Abs(startPoint - leftPoint);
		} else {
			for (int i = 1; i < pointsCount - 1; ++i) {
				_ = Input.ReadInt();
			}
			int rightPoint = Input.ReadInt();
			if (startPoint <= leftPoint) {
				minimalStepsCount = rightPoint - startPoint;
			} else if (startPoint >= rightPoint) {
				minimalStepsCount = startPoint - leftPoint;
			} else {
				int stepsToLeftPoint = startPoint - leftPoint;
				int stepsToRightPoint = rightPoint - startPoint;
				minimalStepsCount = stepsToLeftPoint < stepsToRightPoint
					? stepsToLeftPoint * 2 + stepsToRightPoint
					: stepsToRightPoint * 2 + stepsToLeftPoint;
			}
		}

		Output.WriteLine(minimalStepsCount);
	}
}
