...
/// <summary>
/// Solve https://codeforces.com/problemset/problem/1671/C problem.
/// </summary>
internal class Solver
{
	...
	{
		int shopsCount = Input.ReadInt();
		long everydayBudget = Input.ReadInt();

		int[] prices = Input.ReadIntArray(shopsCount);
		Array.Sort(prices);

		long baseWastedMoney = 0L;
		int lastAvailableShop = -1;
		for (int i = 0; i < shopsCount; ++i) {
			if (baseWastedMoney + prices[lastAvailableShop + 1] <= everydayBudget) {
				baseWastedMoney += prices[++lastAvailableShop];
			} else {
				break;
			}
		}

		long totalPacksCount = 0L;
		long dayNumber = 0;
		while (lastAvailableShop >= 0) {
			int shopsLeft = lastAvailableShop + 1;
			long todayWastedMonet = baseWastedMoney + dayNumber * shopsLeft;
			long daysWithThisShop = 0L;
			if (todayWastedMonet <= everydayBudget) {
				daysWithThisShop = (everydayBudget - todayWastedMonet) / shopsLeft + 1L;
			}
			totalPacksCount += shopsLeft * daysWithThisShop;
			baseWastedMoney -= prices[lastAvailableShop--];
			dayNumber += daysWithThisShop;
		}
		Output.WriteLine(totalPacksCount);
	}
}
