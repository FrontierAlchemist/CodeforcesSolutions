...
/// <summary>
/// Solve https://codeforces.com/contest/1974/problem/A problem.
/// </summary>
internal class Solver
{
	...
	{
		int applicationsWithSmallIconCount = Input.ReadInt();
		int applicationsWithBigIconCount = Input.ReadInt();

		int screensCount = Math.DivRem(applicationsWithBigIconCount, 2, out int remainder);
		if (remainder > 0) {
			++screensCount;
		}

		int freeCellsForSmallIcons = 7 * screensCount + (remainder > 0 ? 4 : 0);
		if (applicationsWithSmallIconCount > freeCellsForSmallIcons) {
			screensCount += Math.DivRem(applicationsWithSmallIconCount - freeCellsForSmallIcons, 15, out remainder);
			if (remainder > 0) {
				++screensCount;
			}
		}
		Output.WriteLine(screensCount);
	}
}
