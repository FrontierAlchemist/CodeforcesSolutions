...
/// <summary>
/// Solve https://codeforces.com/contest/2119/problem/A problem.
/// </summary>
internal class Solver
{
	...
	{
		int sourceNumber = Input.ReadInt();
		int targetNumber = Input.ReadInt();
		int incrementCost = Input.ReadInt();
		int xorCost = Input.ReadInt();

		long minimalTotalCost = 0L;
		if (sourceNumber < targetNumber) {
			if (incrementCost <= xorCost) {
				minimalTotalCost += (targetNumber - sourceNumber) * incrementCost;
			} else {
				while (sourceNumber < targetNumber) {
					minimalTotalCost += (sourceNumber & 1) == 0 ? xorCost : incrementCost;
					++sourceNumber;
				}
			}
		} else if (sourceNumber - targetNumber == 1 && sourceNumber % 2 == 1) {
			minimalTotalCost = xorCost;
