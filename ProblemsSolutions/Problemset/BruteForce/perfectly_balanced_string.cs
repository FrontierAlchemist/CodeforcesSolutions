...
/// <summary>
/// Solve https://codeforces.com/problemset/problem/1673/B problem.
/// </summary>
internal class Solver
{
	...
	private static void Solve()
	{
		...
		Dictionary<char, int> lastPosition = new();
		for (int i = 0; i < str.Length; ++i) {
			if (!lastPosition.ContainsKey(str[i])) {
				lastPosition.Add(str[i], -1);
			}
		}

		bool isPerfectlyBalancedString = true;
		if (lastPosition.Count > 1) {
			int step = lastPosition.Count;
			for (int i = 0; i < str.Length; ++i) {
				if (lastPosition[str[i]] != -1 && i - lastPosition[str[i]] != step) {
					isPerfectlyBalancedString = false;
					break;
				}
		...
		Output.WriteLine(isPerfectlyBalancedString ? "YES" : "NO");
	}
}
