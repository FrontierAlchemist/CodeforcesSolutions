...
/// <summary>
/// Solve https://codeforces.com/contest/2098/problem/A problem.
/// </summary>
internal class Solver
{
	...
	private static void Solve()
	{
		...
		char[] digits = Input.ReadString().ToCharArray();
		for (int i = 0; i < DigitsCount - 1; ++i) {
			int k = -1;
			for (int j = i + 1; j < DigitsCount; ++j) {
				if (digits[j] < digits[i] && digits[j] >= (char)('9' - i)) {
					if (k == -1 || digits[j] < digits[k]) {
						k = j;
					}
				}
			}
		...
		Output.WriteLine(string.Join("", digits));
	}
}
