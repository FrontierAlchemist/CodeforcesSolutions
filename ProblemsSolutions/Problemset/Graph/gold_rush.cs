...
/// <summary>
/// Solve https://codeforces.com/problemset/problem/1829/D problem.
/// </summary>
internal class Solver
{
	...
	{
		int sourcePileSize = Input.ReadInt();
		int targetPileSize = Input.ReadInt();

		Output.WriteLine(TryMakePileWithTargetSize(sourcePileSize) ? "YES" : "NO");

		bool TryMakePileWithTargetSize(int size)
		{
			if (size < targetPileSize) {
				return false;
			}
			if (size == targetPileSize) {
				return true;
			}
			int smallerPileSize = Math.DivRem(size, 3, out int remainder);
			if (smallerPileSize == 0 || remainder != 0) {
				return false;
			}
			return TryMakePileWithTargetSize(smallerPileSize) || TryMakePileWithTargetSize(smallerPileSize * 2);
		}
	}
}
