...
/// <summary>
/// Solve https://codeforces.com/contest/2121/problem/B problem.
/// </summary>
internal class Solver
{
	...
	private static void Solve()
	{
		...
		Dictionary<char, int> lettersFrequencies = new();
		for (int i = 0; i < stringLength; ++i) {
			if (!lettersFrequencies.TryGetValue(sourceString[i], out int frequency)) {
				lettersFrequencies.Add(sourceString[i], 0);
			}
			lettersFrequencies[sourceString[i]] = frequency + 1;
		}
		bool isRequestedString = false;
		for (int i = 1; i < stringLength - 1; ++i) {
			if (lettersFrequencies[sourceString[i]] > 1) {
				isRequestedString = true;
				break;
			}
		}
		Output.WriteLine(isRequestedString ? "YES" : "NO");
	}
}
