...
/// <summary>
/// Solve https://codeforces.com/problemset/problem/1612/C problem.
/// </summary>
internal class Solver
{
	...
	{
		long triangleSize = Input.ReadInt();
		long emotionsLimit = Input.ReadLong();

		long maxMessagesCount = 2L * triangleSize - 1L;
		long maxEmotionsCount = triangleSize * triangleSize;

		long leftPoint = 0L;
		long rightPoint = maxMessagesCount;
		while (leftPoint < rightPoint) {
			long middlePoint = (leftPoint + rightPoint) / 2L;
			long printedMessages = middlePoint;
			long printedEmotions = CalculatePrintedEmotions(printedMessages);

			if (printedEmotions < emotionsLimit) {
		...
		long totalPrintedMessages = rightPoint;
		Output.WriteLine(totalPrintedMessages);

		long CalculatePrintedEmotions(long messages)
		{
			long printedEmotions;
			if (messages <= triangleSize) {
				printedEmotions = messages * (messages + 1L) / 2L;
			} else {
				long notPrintedMessages = maxMessagesCount - messages;
				printedEmotions = maxEmotionsCount - (notPrintedMessages * (notPrintedMessages + 1L) / 2L);
			}
			return printedEmotions;
		}
	}
}
