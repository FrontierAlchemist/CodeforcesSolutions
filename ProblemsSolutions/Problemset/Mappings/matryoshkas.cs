...
/// <summary>
/// Solve https://codeforces.com/problemset/problem/1790/D problem.
/// </summary>
internal class Solver
{
	...
	{
		int dollsCount = Input.ReadInt();
		SortedDictionary<int, int> dolls = new();
		for (int i = 0; i < dollsCount; ++i) {
			int dollSize = Input.ReadInt();
			if (!dolls.ContainsKey(dollSize)) {
				dolls.Add(dollSize, 0);
			}
			++dolls[dollSize];
		}

		int totalDollsSetsCount = 0;
		int lastSize = 0;
		int inBuildingSetsCount = 0;
		foreach ((int currentSize, int dollsCountOfCurrentSize) in dolls) {
			int stoppedBuildedCount = lastSize + 1 == currentSize ? Math.Max(inBuildingSetsCount - dollsCountOfCurrentSize, 0) : inBuildingSetsCount;
			totalDollsSetsCount += stoppedBuildedCount;
			inBuildingSetsCount = dollsCountOfCurrentSize;
			lastSize = currentSize;
		}
		totalDollsSetsCount += inBuildingSetsCount;

		Output.WriteLine(totalDollsSetsCount);
	}
}
