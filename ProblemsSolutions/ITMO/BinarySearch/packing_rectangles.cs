...
public class Program
{
	...
	static void SolveTestCase()
	{
		...
		long rectangleWidth = long.Parse(valuesFromInput[0]);
		long rectangleHeight = long.Parse(valuesFromInput[1]);
		long rectanglesCount = long.Parse(valuesFromInput[2]);

		long leftBorder = 0;
		long rightBorder = 1;
		while (!IsSquareCanPackAllRectangles(rightBorder)) {
			rightBorder <<= 1;
		}
		while (leftBorder + 1 < rightBorder) {
			long checkedWidth = (leftBorder + rightBorder) / 2;
			if (IsSquareCanPackAllRectangles(checkedWidth)) {
				rightBorder = checkedWidth;
			} else {
				leftBorder = checkedWidth;
			}
		}

		writer.WriteLine(rightBorder.ToString());

		bool IsSquareCanPackAllRectangles(long squareWidth)
		{
			long packedRectanglesCount = (squareWidth / rectangleWidth) * (squareWidth / rectangleHeight);
			return packedRectanglesCount >= rectanglesCount;
		}
	}
