...
namespace CodeForcesCSharpTemplate
{
	...
	public class Program
	{
		...
		static void SolveTestCase()
		{
			...
			int arrayLength = int.Parse(valuesFromConsole[0]);
			int queiriesCount = int.Parse(valuesFromConsole[1]);
			...
			for (int i = 0; i < queiriesCount; ++i) {
				int target = int.Parse(valuesFromConsole[i]);
				readerWriter.WriteLine(IsElementExist(target) ? "YES" : "NO");
			}

			bool IsElementExist(int target)
			{
				int leftBound = 0;
