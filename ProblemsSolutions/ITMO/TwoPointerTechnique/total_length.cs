...
/// <summary>
/// Solve https://codeforces.com/edu/course/2/lesson/9/3/practice/contest/307094/problem/B problem.
/// </summary>
public class ProblemSolver
{
	...
	private void SolveTestCase()
	{
		...
		int leftBorder = 0;
		long currentSum = 0L;
		long totalLengthOfValidSegments = 0L;
		for (int rightBorder = 0; rightBorder < arraySize; ++rightBorder) {
			currentSum += array[rightBorder];
			while (currentSum > sumLimit) {
				currentSum -= array[leftBorder++];
			}
			long segmentLength = rightBorder - leftBorder + 1;
			totalLengthOfValidSegments += segmentLength * (segmentLength + 1) / 2;
		}

		output.WriteLine(totalLengthOfValidSegments);
	}
}
