	...
	static void SolveTestCase()
	{
		...
		while (firstPointer < firstArraySize && secondPointer < secondArraySize) {
			if (firstArray[firstPointer] < secondArray[secondPointer]) {
				...
				long equalNumbersCount = 0;
				while (firstPointer < firstArraySize && firstArray[firstPointer] == secondArray[secondPointer]) {
					++firstPointer;
					++equalNumbersCount;
				}
				long multiplier = 1;
				while (++secondPointer < secondArraySize && secondArray[secondPointer] == secondArray[secondPointer - 1]) {
					++multiplier;
				}
				totalEqualNumbersCount += multiplier * equalNumbersCount;
			}
		}

		writer.WriteLine(totalEqualNumbersCount.ToString());
	}
