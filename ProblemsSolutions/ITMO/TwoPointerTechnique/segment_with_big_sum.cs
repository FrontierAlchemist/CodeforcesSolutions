...
public class Program
{
	...
	static void SolveTestCase()
	{
		...
		int smallestSegmentLength = arraySize + 1;
		long currentSum = 0L;
		int left = 0;
		for (int right = 0; right < arraySize; ++right) {
			...
			if (currentSum >= sumLimit) {
				...
				smallestSegmentLength = Math.Min(segmentLength, smallestSegmentLength);
			}
		}

		writer.WriteLine(smallestSegmentLength == arraySize + 1 ? -1 : smallestSegmentLength);
	}
