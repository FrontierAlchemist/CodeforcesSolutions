...
public class Program
{
	...
	static void SolveTestCase()
	{
		...
		int mergredArraySize = firstArraySize + secondArraySize;
		int[] mergedArray = new int[mergredArraySize];
		int firstPointer = 0;
		int secondPointer = 0;
		while (firstPointer < firstArraySize || secondPointer < secondArraySize) {
			if (
				secondPointer == secondArraySize ||
				(firstPointer < firstArraySize && firstArray[firstPointer] < secondArray[secondPointer])
			) {
				mergedArray[firstPointer + secondPointer] = firstArray[firstPointer];
				++firstPointer;
			} else {
				mergedArray[firstPointer + secondPointer] = secondArray[secondPointer];
				++secondPointer;
			}
		}
